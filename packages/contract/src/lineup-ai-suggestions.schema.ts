 ...
 *
 * Produced by `GET /lineups/:id/suggestions`. Server-enriched with
 * `name`/`coverUrl`/`ownershipCount`/`voterTotal` on top of what the LLM
 * returns (`gameId` + `confidence` + `reasoning`).
 */
