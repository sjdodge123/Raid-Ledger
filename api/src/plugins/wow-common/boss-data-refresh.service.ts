...
import {
  BASE_URL, NAMESPACE, EXPANSION_KEY,
  type JournalInstance, type JournalEncounter,
  upsertBoss, processLootItem,
...
export class BossDataRefreshService {
  ...
  async refresh(): Promise<{ bosses: number; loot: number }> {
    if (this.isRefreshing) { this.logger.warn('Boss data refresh already in progress, skipping'); return { bosses: 0, loot: 0 }; }
    ...
    try {
      ...
      for (const inst of instances) {
        ...
        } catch (err) { this.logger.warn(`Failed to refresh instance ${inst.id}: ${err}`); }
        await this.sleep(100);
      }

      await this.bossEncountersService.clearCache();
      await this.dungeonQuestsService.clearCache();
      this.logger.log(`Boss data refresh complete: ${totalBosses} bosses, ${totalLoot} loot items`);
      return { bosses: totalBosses, loot: totalLoot };
    } finally { this.isRefreshing = false; }
  }

  /** Fetch TBC instance IDs from the Blizzard journal. */
  private async fetchAllInstanceIds(): Promise<Array<{ id: number; expansion: string }>> {
    const result: Array<{ id: number; expansion: string }> = [];
    const targetExpansions = new Set(['Burning Crusade']);

    const index = await this.blizzardService.fetchBlizzardApi<{ tiers: Array<{ id: number; name: string }> }>(
      `${BASE_URL}/data/wow/journal-expansion/index?namespace=${NAMESPACE}&locale=en_US`,
    );
    if (!index) return result;

    for (const tier of index.tiers) {
      const detail = await this.blizzardService.fetchBlizzardApi<{ name: string; dungeons?: Array<{ id: number; name: string }>; raids?: Array<{ id: number; name: string }> }>(
      ...
      for (const d of detail.dungeons || []) result.push({ id: d.id, expansion });
  ...
  private async refreshInstance(instanceId: number, expansion: string): Promise<{ bosses: number; loot: number }> {
    const journal = await this.blizzardService.fetchBlizzardApi<JournalInstance>(
    ...
    let bossCount = 0, lootCount = 0;
    for (let i = 0; i < journal.encounters.length; i++) {
      const enc = journal.encounters[i];
      const bossId = await upsertBoss(this.db, instanceId, enc.name, i + 1, expansion);
      bossCount++;
      if (!bossId) continue;
  ...
  private async processEncounterLoot(encounterId: number, bossId: number, expansion: string): Promise<number> {
    const encDetail = await this.blizzardService.fetchBlizzardApi<JournalEncounter>(
    ...
    for (const itemEntry of encDetail.items) {
      ...
      try {
        const upserted = await processLootItem(this.db, this.blizzardService, bossId, itemEntry.item, expansion);
