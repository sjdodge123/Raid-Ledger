  ...
  describe('nullable fields', () => {
    ...
    it('classRestrictions is either null or a non-empty array of strings', () => {
      const bad = loot.filter((l) => {
        ...
        if (l.classRestrictions.length === 0) return true;
...
function expectAllBossesHaveLoot(expansion: string) {
  const expBosses = bosses.filter((b) => b.expansion === expansion);
  const expLootBosses = new Set(
    loot.filter((l) => l.expansion === expansion).map((l) => l.bossName),
  );
  const missing = expBosses.filter((b) => !expLootBosses.has(b.name));
  expect(missing).toHaveLength(0);
}

function testIconUrlDomains() {
...
function testExpansionConsistency() {
  ...
  const mismatched = loot.filter((l) => {
    ...
    return encounterMap.get(key) === undefined;
  });
  expect(mismatched).toHaveLength(0);
}

function testWindseekerBindings() {
...
function testScarletMonasteryLoot() {
  const smBosses = [
    'Interrogator Vishas',
    'Bloodmage Thalnos',
    'Houndmaster Loksey',
...
describe('boss-loot-data.json — completeness against boss-encounter-data.json', () => {
  ...
  describe('specific classic bosses', () => {
    it('Majordomo Executus (Classic) has loot', () => {
    ...
    it('Scarlet Monastery bosses all have loot', () =>
      testScarletMonasteryLoot());
  });

  describe('per-expansion coverage', () => {
    it('SoD bosses all have loot', () => {
      expectAllBossesHaveLoot('sod');
    });

    it('TBC bosses all have loot', () => {
      expectAllBossesHaveLoot('tbc');
    });

    it('WotLK bosses all have loot', () => {
      expectAllBossesHaveLoot('wotlk');
    });

    it('Cata bosses all have loot', () => {
      expectAllBossesHaveLoot('cata');
    });
  });
});
...
describe('boss-loot-data.json — data quality', () => {
  describe('field validity', () => {
    ...
    it('all expansion values match the corresponding seeded boss', () => {
      testExpansionConsistency();
    });
  });

  describe('specific item checks', () => {
...
describe('boss-loot-data.json — regression (existing loot not removed)', () => {
  describe('specific items still present', () => {
