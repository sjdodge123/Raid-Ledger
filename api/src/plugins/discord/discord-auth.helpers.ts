...
export function verifyOAuthState(state: string, secret: string, logger: Logger): Record<string, unknown> | null {
  try {
    const { data, signature } = JSON.parse(Buffer.from(state, 'base64').toString()) as { data: string; signature: string };
...
export async function exchangeCodeForToken(
  ...
  const tokenResponse = await discordFetch('https://discord.com/api/oauth2/token', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': 'RaidLedger (https://github.com/sjdodge123/Raid-Ledger, 1.0)' },
...
export async function fetchDiscordProfile(
  ...
  const userResponse = await discordFetch('https://discord.com/api/users/@me', {
    headers: { Authorization: `Bearer ${accessToken}`, 'User-Agent': 'RaidLedger (https://github.com/sjdodge123/Raid-Ledger, 1.0)' },
  });
  if (!userResponse.ok) throw new Error('Failed to fetch Discord profile');
  return (await userResponse.json()) as { id: string; username: string; avatar?: string };
}
