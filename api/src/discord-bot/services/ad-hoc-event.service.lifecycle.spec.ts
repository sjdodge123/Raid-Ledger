...
describe('AdHocEventService — lifecycle', () => {
  ...
  async function createActiveEvent(bindingId: string, eventId: number) {
    ...
    mocks.db.limit.mockResolvedValueOnce([{ id: eventId, title: 'WoW — Quick Play', gameId: 1, channelBindingId: bindingId }]);
    mocks.db.limit.mockResolvedValueOnce([{ name: 'WoW' }]);
  ...
  describe('finalizeEvent', () => {
    it('finalizes event when status is grace_period', async () => {
      mocks.db.returning.mockResolvedValueOnce([{
        id: 500, adHocStatus: 'grace_period', channelBindingId: 'binding-fin', gameId: 1,
      ...
      expect(mocks.db.set).toHaveBeenCalledWith(expect.objectContaining({ adHocStatus: 'ended' }));
    });
    ...
    it('removes binding from active events map', async () => {
      ...
      mocks.db.returning.mockResolvedValueOnce([{
        id: 600, adHocStatus: 'grace_period', channelBindingId: 'binding-cleanup', gameId: 1,
  ...
  describe('getAdHocRoster', () => {
    it('returns roster with active count', async () => {
      mocks.participantService.getRoster.mockResolvedValue([{
        id: 'uuid-1', eventId: 42, userId: 1, discordUserId: 'discord-1',
        discordUsername: 'Player1', discordAvatarHash: null,
        joinedAt: '2026-02-10T18:00:00Z', leftAt: null,
        totalDurationSeconds: null, sessionCount: 1,
      }]);
      mocks.participantService.getActiveCount.mockResolvedValue(1);
      const result = await service.getAdHocRoster(42);
      expect(result).toMatchObject({ eventId: 42, participants: expect.any(Array), activeCount: 1 });
  ...
  describe('onModuleInit', () => {
    it('recovers live ad-hoc events from database', async () => {
      mocks.db.where.mockResolvedValueOnce([
        { id: 800, isAdHoc: true, adHocStatus: 'live', channelBindingId: 'binding-800' },
        { id: 801, isAdHoc: true, adHocStatus: 'live', channelBindingId: 'binding-801' },
      ]);
    ...
    it('skips events without channelBindingId', async () => {
      mocks.db.where.mockResolvedValueOnce([{ id: 900, isAdHoc: true, adHocStatus: 'live', channelBindingId: null }]);
      await service.onModuleInit();
  ...
  describe('scheduled event interaction', () => {
    it('extends scheduled event end time when members join during active event', async () => {
      ...
      mocks.db.limit.mockResolvedValueOnce([{ id: 42, duration: [eventStart, eventEnd] }]);
      await service.handleVoiceJoin('binding-extend', baseMember, baseBinding);
      expect(mocks.db.insert).not.toHaveBeenCalled();
      expect(mocks.db.update).toHaveBeenCalled();
      expect(mocks.db.set).toHaveBeenCalledWith(expect.objectContaining({ extendedUntil: expect.any(Date) }));
    });
    ...
    it('allows ad-hoc creation when no scheduled event exists', async () => {
      ...
      mocks.db.limit.mockResolvedValueOnce([{ id: 999, title: 'WoW — Quick Play', gameId: 1, channelBindingId: 'binding-new' }]);
      mocks.db.limit.mockResolvedValueOnce([{ name: 'WoW' }]);
