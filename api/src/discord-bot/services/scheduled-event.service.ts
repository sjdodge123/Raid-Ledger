...
export class ScheduledEventService {
  ...
  async createScheduledEvent(
    ...
    try {
      ...
      const voiceChannelId = notificationChannelOverride ??
        await this.channelResolver.resolveVoiceChannelForScheduledEvent(gameId, eventRow?.recurrenceGroupId);
      if (!voiceChannelId) {
        this.logger.warn(
  ...
  async updateScheduledEvent(
    ...
    try {
      ...
      // Get stored scheduled event ID + channel override + recurrence group
