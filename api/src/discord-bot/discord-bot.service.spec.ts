...
beforeEach(async () => {
  ...
  clientService = module.get<DiscordBotClientService>(DiscordBotClientService);
...
describe('DiscordBotService — onApplicationBootstrap', () => {
...
describe('DiscordBotService — onModuleDestroy', () => {
...
describe('DiscordBotService — handleConfigUpdate', () => {
...
describe('DiscordBotService — ensureConnected', () => {
...
describe('DiscordBotService — sendDm', () => {
...
describe('DiscordBotService — getStatus: connected', () => {
...
describe('DiscordBotService — getStatus: not connected', () => {
...
/** Shared setup: mock all settings checks as false/null for getSetupStatus. */
function mockAllSettingsUnconfigured() {
  jest.spyOn(settingsService, 'isDiscordConfigured').mockResolvedValue(false);
  jest
  ...
  jest.spyOn(settingsService, 'getDiscordBotTimezone').mockResolvedValue(null);
}

describe('DiscordBotService — getSetupStatus: fully configured', () => {
  it('should return all steps completed when everything is configured', async () => {
    jest.spyOn(settingsService, 'isDiscordConfigured').mockResolvedValue(true);
    jest
      .spyOn(settingsService, 'isDiscordBotConfigured')
...
describe('DiscordBotService — getSetupStatus: unconfigured', () => {
  it('should return all steps incomplete when nothing is configured', async () => {
    mockAllSettingsUnconfigured();
...
describe('DiscordBotService — getSetupStatus: partial', () => {
...
describe('DiscordBotService — testToken: success', () => {
  ...
  it('should disconnect test client even on success', async () => {
    ...
    jest
      .spyOn(DiscordBotClientService.prototype, 'getGuildInfo')
      .mockReturnValue({ name: 'Guild', memberCount: 10 });
...
describe('DiscordBotService — testToken: failures', () => {
  ...
  it('should return friendly message for ECONNREFUSED', async () => {
    ...
    expect(result).toEqual({
      success: false,
      message: 'Connection to Discord was refused. Try again in a few moments.',
    });
  });
