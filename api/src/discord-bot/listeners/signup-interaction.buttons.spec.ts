...
async function setupButtonsModule() {
  delete process.env.CLIENT_URL;
  mocks = await createSignupInteractionTestModule();
}

async function teardownButtonsModule() {
  jest.clearAllMocks();
...
function linkedUserSignupTests() {
...
function tentativeToSignedUpTest() {
...
function unlinkedUserTests() {
  it('should show onboarding ephemeral for unlinked Discord user', async () => {
    ...
    mocks.mockDb.select.mockReturnValueOnce({
      from: jest.fn().mockReturnValue({
        where: jest.fn().mockReturnValue({
          limit: jest.fn().mockResolvedValue([{ id: 1, title: 'Raid Night' }]),
        }),
      }),
    });
...
function quickSignupTests() {
  it('should create anonymous signup for non-MMO event', async () => {
    ...
    expect(interaction.editReply).toHaveBeenCalledWith(
      expect.objectContaining({ content: expect.stringContaining('TestUser') }),
    );
  });
...
describe('SignupInteractionListener — buttons', () => {
  beforeEach(async () => {
    await setupButtonsModule();
  });

  afterEach(async () => {
    await teardownButtonsModule();
  });
  ...
  describe('handleButtonInteraction — signup button (linked user)', () => {
    linkedUserSignupTests();
    tentativeToSignedUpTest();
    unlinkedUserTests();
  });
