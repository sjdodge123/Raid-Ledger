...
function setupEventLinkModule() {
  process.env.CLIENT_URL = 'http://localhost:5173';

  mockClientService = { getClient: jest.fn() };
...
describe('EventLinkListener', () => {
  ...
  describe('message handling — basic', () => {
    messageBasicTests();
  });

  describe('message handling — filtering', () => {
    messageFilterTests();
  });

  describe('message handling — edge cases', () => {
    messageEdgeCaseTests();
  });
});
...
function messageEdgeCaseTests() {
  ...
  it('should handle embed without row when no CLIENT_URL for preview', async () => {
    ...
    const msg = createMessage('http://localhost:5173/events/42');
    await callHandleMessage(msg);
    expect(mockReply).toHaveBeenCalledWith({ embeds: [mockEmbed] });
  });
