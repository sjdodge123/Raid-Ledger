...
describe('LFG affinity DMs (integration)', () => {
  ...
  it('writes exactly one invite per (game, user) across two waves (T14)', async () => {
    ...
    await service.handleLfmReached({ gameId: game.id, activeCount: 2, urgency: 'week' });
    await service.handleLfmReached({ gameId: game.id, activeCount: 3, urgency: 'week' });
  ...
  it('keys the dedup per game — a second game invites again', async () => {
    ...
    await service.handleLfmReached({ gameId: first.id, activeCount: 2, urgency: 'week' });
    await service.handleLfmReached({ gameId: second.id, activeCount: 2, urgency: 'week' });
