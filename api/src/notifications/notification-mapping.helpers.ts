 ...
 */
import { DEFAULT_CHANNEL_PREFS, type ChannelPrefs } from '../drizzle/schema/notification-preferences';
import * as schema from '../drizzle/schema';
import type { Channel, NotificationDto, NotificationPreferencesDto } from './notification.types';

/** Map a notifications DB row to a DTO. */
export function mapNotificationToDto(row: typeof schema.notifications.$inferSelect): NotificationDto {
  return {
    id: row.id,
    userId: row.userId,
    type: row.type,
...
export function mapPreferencesToDto(row: typeof schema.userNotificationPreferences.$inferSelect): NotificationPreferencesDto {
  ...
  for (const [type, channels] of Object.entries(stored)) {
    ...
    if (merged[notifType] && channels) {
      merged[notifType] = { ...merged[notifType], ...(channels as Record<Channel, boolean>) };
    }
  }
