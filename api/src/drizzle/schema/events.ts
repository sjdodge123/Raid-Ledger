import { pgTable, serial, text, timestamp, check, customType, integer } from 'drizzle-orm/pg-core';
...
export const tsrange = customType<{
    ...
    fromDriver(value: string) {
        ...
        return [new Date(matches[1].replace(/"/g, '')), new Date(matches[2].replace(/"/g, ''))];
    },
...
export const events = pgTable('events', {
    ...
    gameId: text('game_id'), // Reference to IGDB
