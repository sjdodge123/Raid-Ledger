...
describe('OgMetaService — security & structure', () => {
  ...
  describe('XSS prevention', () => {
    it('should escape double quotes in event title', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: 'Title with "quotes"', game: null }));
    ...
    it('should escape single quotes in event title', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: "O'Reilly's Bash", game: null }));
    ...
    it('should escape angle brackets in event title', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: '<Evil> & "Nasty"', game: null }));
    ...
    it('should escape ampersands in event title', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: 'Raid & Conquer', game: null }));
    ...
    it('should escape ampersands in game name', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ game: { name: 'Dungeons & Dragons', coverUrl: null } }));
    ...
    it('should escape script tags in game cover URL', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ game: { name: 'Test Game', coverUrl: 'javascript:alert("xss")' } }));
      const html = await service.renderInviteOgHtml('xss6');
      expect(html).not.toContain('alert("xss")');
    });

    it('should escape angle brackets in game cover URL', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ game: { name: 'Test Game', coverUrl: 'https://cdn.example.com/<evil>.jpg' } }));
    ...
    it('should handle unicode characters without escaping them', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: 'Caf\u00e9 Raider \u2014 \u014Ckami Night', game: null }));
    ...
    it('should not render a raw <script> tag anywhere in the output', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: '<script>document.cookie</script>', game: null }));
  ...
  describe('Edge cases — long strings and missing fields', () => {
    it('should handle a very long event title without throwing', async () => {
      ...
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: longTitle, game: null }));
      const html = await service.renderInviteOgHtml('long1');
      expect(html).toContain(longTitle);
    });

    it('should handle an event with no startTime', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: true, event: { id: 1, title: 'Open Night', startTime: undefined, endTime: undefined, game: null }, slot: { id: 1, role: 'dps', status: 'pending' } });
    ...
    it('should handle game with empty string cover URL as missing cover art', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ game: { name: 'Unknown Game', coverUrl: null } }));
    ...
    it('should include "Game:" line in description when game name is present', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ game: { name: 'Elden Ring', coverUrl: null } }));
    ...
    it('should handle resolve returning valid:true but event is null/undefined', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: true, event: null });
  ...
  describe('Fallback OG tags per invalid invite status', () => {
    ...
    it('expired error gets "already ended" description', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'Event has expired' });
    ...
    it('"ended" keyword in error maps to "already ended" description', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'Event has ended' });
      const html = await service.renderInviteOgHtml('bad4');
      expect(html).toContain('already ended');
    });

    it('claimed error gets "already been claimed" description', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'Slot claimed' });
    ...
    it('cancelled error gets "been cancelled" description', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'Event cancelled by organizer' });
    ...
    it('case-insensitive matching: mixed-case "Expired" keyword triggers ended message', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'EXPIRED link' });
    ...
    it('case-insensitive matching: mixed-case "Cancelled" keyword triggers cancelled message', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'CANCELLED by admin' });
  ...
  describe('Meta refresh redirect tag', () => {
    it('should have http-equiv="refresh" pointing to canonical URL for valid invite', async () => {
      ...
      expect(html).toMatch(/http-equiv="refresh" content="0;url=https:\/\/raid\.example\.com\/i\/refresh1"/);
    });

    it('should have http-equiv="refresh" for fallback pages too', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue({ valid: false, error: 'Not found' });
  ...
  describe('HTML structure validity', () => {
    ...
    it('should have a <title> tag matching og:title', async () => {
      mocks.inviteService.resolveInvite.mockResolvedValue(makeValidInvite({ title: 'Struct Event', game: null }));
  ...
  describe('InviteController — OG endpoint integration', () => {
    it('should return the HTML produced by OgMetaService', async () => {
      ...
      const mockInviteService = { resolveInvite: jest.fn(), claimInvite: jest.fn() };
    ...
    it('renderOgMeta route is registered before :code to avoid shadowing', () => {
      const proto = InviteController.prototype as unknown as Record<string, unknown>;
