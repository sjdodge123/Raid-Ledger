...
describe('SignupsService — signupDiscord', () => {
  ...
  describe('signupDiscord', () => {
    it('should create anonymous signup for unlinked Discord user', async () => {
      mockDb.select.mockReturnValueOnce(makeSelectChain([mockEvent])).mockReturnValueOnce(makeSelectChain([]));
      const result = await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser', discordAvatarHash: 'avatar-hash-abc' });
    ...
    it('should use normal signup path when Discord user has a linked RL account', async () => {
      const signupSpy = jest.spyOn(service, 'signup').mockResolvedValueOnce({
        id: 11, eventId: 1, user: { id: 1, discordId: 'discord-user-123', username: 'linkeduser', avatar: null },
        note: null, signedUpAt: new Date().toISOString(), characterId: null, character: null, confirmationStatus: 'pending', status: 'signed_up',
      });
      mockDb.select.mockReturnValueOnce(makeSelectChain([mockEvent])).mockReturnValueOnce(makeSelectChain([mockUser]));
      await service.signupDiscord(1, { discordUserId: 'discord-user-123', discordUsername: 'linkeduser' });
      expect(signupSpy).toHaveBeenCalledWith(1, mockUser.id, { preferredRoles: undefined, slotRole: undefined });
    });

    it('should throw NotFoundException when event does not exist', async () => {
      mockDb.select.mockReturnValueOnce(makeSelectChain([]));
      await expect(service.signupDiscord(999, { discordUserId: 'discord-anon', discordUsername: 'AnonUser' })).rejects.toThrow(NotFoundException);
    });
    ...
    it('should return existing anonymous signup on duplicate (onConflictDoNothing empty)', async () => {
      ...
      const result = await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser' });
    ...
    it('should create roster assignment when role is provided', async () => {
      ...
      mockDb.insert
        .mockReturnValueOnce({
          values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([mockAnonymousSignup]) }) }),
        })
        .mockReturnValueOnce({ values: jest.fn().mockResolvedValue(undefined) });
      await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser', role: 'dps' });
    ...
    it('should set status to tentative when dto.status is tentative', async () => {
      ...
      mockDb.select.mockReturnValueOnce(makeSelectChain([mockEvent])).mockReturnValueOnce(makeSelectChain([]));
      mockDb.insert.mockReturnValueOnce({
        values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([tentativeSignup]) }) }),
      });
      const result = await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser', status: 'tentative' });
      expect(result.status).toBe('tentative');
    });
  });

  describe('signupDiscord — ROK-451 generic auto-slot', () => {
    const genericEvent = { ...mockEvent, slotConfig: { type: 'generic', player: 4, bench: 2 } };
    const mmoEvent = { ...mockEvent, slotConfig: { type: 'mmo', tank: 2, healer: 4, dps: 14 } };
    ...
    it('should auto-assign to player slot for generic event without explicit role', async () => {
      mockDb.select
        ...
        .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: jest.fn().mockResolvedValue([]) }) });
      mockDb.insert
        .mockReturnValueOnce({ values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([mockAnonymousSignup]) }) }) })
        .mockReturnValueOnce({ values: jest.fn().mockResolvedValue(undefined) });
      await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser' });
      expect(mockDb.insert).toHaveBeenCalledTimes(2);
    });

    it('should NOT auto-slot for MMO events (role selection required)', async () => {
      mockDb.select.mockReturnValueOnce(makeSelectChain([mmoEvent])).mockReturnValueOnce(makeSelectChain([]));
      mockDb.insert.mockReturnValueOnce({ values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([mockAnonymousSignup]) }) }) });
      await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser' });
    ...
    it('should NOT auto-slot when all generic player slots are full', async () => {
      mockDb.select
        .mockReturnValueOnce(makeSelectChain([genericEvent]))
        .mockReturnValueOnce(makeSelectChain([]))
        .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: jest.fn().mockResolvedValue([{ position: 1 }, { position: 2 }, { position: 3 }, { position: 4 }]) }) });
      mockDb.insert.mockReturnValueOnce({ values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([mockAnonymousSignup]) }) }) });
      await service.signupDiscord(1, { discordUserId: 'discord-anon-456', discordUsername: 'AnonUser' });
      expect(mockDb.insert).toHaveBeenCalledTimes(1);
    });
    ...
    it('should auto-slot when event has maxAttendees but no slotConfig', async () => {
      mockDb.select
        ...
        .mockReturnValueOnce(makeSelectChain([]))
        .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: jest.fn().mockResolvedValue([{ position: 1 }, { position: 2 }]) }) })
        .mockReturnValueOnce({ from: jest.fn().mockReturnValue({ where: jest.fn().mockResolvedValue([{ position: 1 }, { position: 2 }]) }) });
      mockDb.insert
        .mockReturnValueOnce({ values: jest.fn().mockReturnValue({ onConflictDoNothing: jest.fn().mockReturnValue({ returning: jest.fn().mockResolvedValue([mockAnonymousSignup]) }) }) })
        .mockReturnValueOnce({ values: jest.fn().mockResolvedValue(undefined) });
