...
| 34 | Nominator without Steam linked → nomination allowed with warning (private lineup flow) | `web/src/components/lineups/SteamNudgeBanner.tsx` (component); covered by `SteamNudgeBanner.test.tsx` vitest | full | N/A (covered by vitest unit + Chrome MCP) | _(filled by Phase F drive)_ | N/A |
| 35 | Vote submitted within 1s of deadline cron firing → counted or rejected? Document expected. | `api/src/lineups/lineup-deadline-vote-race.integration.spec.ts` (added by ROK-1068 Phase F) | full | N/A (integration spec) | Not Chrome-MCP-driveable: timing race documented in integration spec. Observed contract: vote BEFORE flip → counted (HTTP 200); vote AFTER flip → rejected (HTTP 400 "voting is only allowed in voting status"); concurrent race → vote can be silently retained on a now-decided lineup (vote path reads status outside any transaction). | N/A |
| 36 | Abort with reason vs without → embed copy differs correctly (Discord) | `tools/test-bot/src/smoke/tests/lineup-abort.test.ts` (extended by ROK-1068 Phase F) | full | N/A (companion bot domain) | Not Chrome-MCP-driveable: assertion is Discord embed body via companion bot. Web-side reason field is covered by AC 24. | _(filled by Phase F smoke run)_ |
