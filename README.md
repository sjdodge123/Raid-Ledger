...
**`https://…` fails, or `http://…` loads and then stays blank.** The container speaks plain HTTP, so open it with **`http://`** — `https://<nas-ip>:8080` has nothing to answer it. Want HTTPS? Put it behind a TLS reverse proxy (on a Synology: Control Panel → Login Portal → Advanced → Reverse Proxy, source `https://<your-host>` → destination `http://localhost:8080`, with the NAS certificate) — the app detects the proxy's `X-Forwarded-Proto` and turns on its HTTPS-only security headers. Behind a proxy the API rate-limits each visitor by the address in `X-Forwarded-For`, and by default it trusts only private-network hops (`TRUST_PROXY=loopback, linklocal, uniquelocal` — the Docker bridge, a Cloudflare Tunnel, the NAS reverse proxy). If a proxy in front of the container has a public address, append its IP/CIDR to the defaults — `TRUST_PROXY=loopback, linklocal, uniquelocal, <proxy-ip>` — or every visitor shares one rate limit. Setting `TRUST_PROXY` replaces the default list rather than adding to it, so a value without `loopback, linklocal, uniquelocal` stops at the container's own hops. A hop count is accepted too, but the right number depends on the path into the container (whether the NAS reverse proxy sits in between, for one), and a count that is one too high lets a visitor forge `X-Forwarded-For` — prefer the list form, which is right for every path. Avoid `TRUST_PROXY=true`: it trusts every hop, so a visitor can forge `X-Forwarded-For` and sidestep the limit. **Cloudflare without a tunnel** (a router port-forward to the NAS reverse proxy) is the usual case: the last public hop is a Cloudflare edge server, so append [Cloudflare's IP ranges](https://www.cloudflare.com/ips/) to the defaults, or visitors are limited per Cloudflare edge rather than per person.
