...
| `mcp__mcp-rl-fleet__rl_test_plan_status` | Read current state for the slug's plan: per-step verdicts, summary counts (incl. `pending_resets`, `comment_count`), and `submissions[]` (one entry per tester's Submit action). **Comment bodies are STRIPPED server-side** — agents see `comment_count` and per-step `{tester, ts, has_body}` metadata only. Operator pulls full bodies via `./scripts/test-plan-comments.sh <slug>` for Linear review. This is intentional: tester comments are free-form text and would be an LLM-injection surface if surfaced raw. Polling-friendly + cheap. |
