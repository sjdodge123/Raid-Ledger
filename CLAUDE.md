...
| `mcp__mcp-rl-fleet__rl_env_spin` | Bring up a per-test env (allinone + sibling Postgres). Returns `url` (canonical/shareable — external when `RL_PUBLIC_DOMAIN` is set, internal otherwise), `internal_url` (LAN fallback), `public_url`. Send `url` to testers — Pi-hole short-circuits the LAN path so it's fast on home network AND reachable off-LAN through Cloudflare. |
...
| `mcp__mcp-rl-fleet__rl_test_plan_create` | After `rl_env_deploy`, post a structured test checklist tied to the slug. Steps render on `https://fleet.gamernight.net` (the env card's expandable section) with pass/fail/skip buttons for the operator + external testers. Sequential ordering enforced server-side. Use this so testers know what to verify and the agent gets structured verdicts back. |
| `mcp__mcp-rl-fleet__rl_test_plan_status` | Read current verdicts + summary counts for the slug's plan. Polling-friendly (cheap). |
| `mcp__mcp-rl-fleet__rl_test_plan_wait` | Long-poll via SSH inotifywait — blocks until any tester taps a verdict OR until timeout (default 600s). Push-like UX; use in a loop after `rl_test_plan_create` to react to verdicts as they come in. |
