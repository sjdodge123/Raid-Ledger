...
| `mcp__mcp-rl-fleet__rl_env_spin` | Bring up a per-test env (allinone + sibling Postgres). Returns `url` (canonical), `internal_url` (LAN fallback), `public_url`, `slot_url` (slot-stable hostname for Discord OAuth — `https://slot-N.gamernight.net`), `admin_email` (always `admin@local`), `admin_password` (seeded automatically; stable if `RL_ADMIN_PASSWORD` is in `/srv/rl-infra/.env`, random per-call otherwise). POST `{email, password}` to `{url}/api/auth/local` for a JWT. |
