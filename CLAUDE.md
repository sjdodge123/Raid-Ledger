...
| `mcp__mcp-rl-fleet__rl_task_logs` | Tail the supervisor log for a task: `/srv/rl-infra/state/tasks/<id>.log` (stdout+stderr of the wrapped command). Companion to `rl_task_status` (summarized) and `rl_task_inspect` (raw JSON). `lines` defaults to 100, max 5000. `follow:true` deferred to v2 — returns `error:"follow_not_implemented_in_v1"` for now; poll via `rl_task_status` if you need streaming. Read-only. ROK-1338 PR-2. |
| `mcp__mcp-rl-fleet__rl_env_inspect` | Render the actual contents of a config file inside a fleet env's allinone container. `what` enum: `nginx-conf` (Alpine `/etc/nginx/http.d/default.conf`) or `supervisor-conf` (`/etc/supervisor.d/raid-ledger.ini`). 64KB cap, `truncated:true` on overflow. Routes via rl-docker-proxy at 127.0.0.1:2375 (rl-agent not in docker group). Read-only. ROK-1338 PR-2. |
| `mcp__mcp-rl-fleet__rl_db_query` | Run a one-shot read-only SQL query against a fleet env's Postgres via `env-psql`. Layered safety: PGOPTIONS GUC (`default_transaction_read_only=on` + 5s statement timeout) + BEGIN/SET TRANSACTION READ ONLY/ROLLBACK wrapper + FORBIDDEN_KEYWORDS pre-check + `SELECT * FROM (<your-sql>) AS rl_user_query LIMIT 1001` subquery wrap + `-v ON_ERROR_STOP=1`. Returns rows as `[{col: val,...}]` parsed from CSV with NULL distinguishable from empty string. Caps at 1000 rows (`truncated:true` flag). v1 is read-only only — write mode is a future tool. ROK-1338 PR-2. |
