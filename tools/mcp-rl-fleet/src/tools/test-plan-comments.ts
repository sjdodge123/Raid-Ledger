...
// NFKC-normalises, drops control characters except newline and tab, replaces
// every "<" with U+2039 and ">" with U+203A (no tag can form), caps each body
// at 500 characters and the response at 4000, and emits
...
/**
 * NFKC-normalise (folds fullwidth/small-form brackets to ASCII first), drop
 * control characters except newline and tab, then replace every remaining
 * "<" / ">" so no tag — opening or closing — can form in the output.
 */
...
export function renderCommentBodies(body: unknown): unknown {
  const budget: Budget = { remaining: COMMENT_TOTAL_CAP };
  return mapPlans(body, (plan) =>
