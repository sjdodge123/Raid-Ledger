...
import { schedulingUnanimousDmTests } from "./tests/scheduling-unanimous-dm.test.js";
...
async function runTest(test: SmokeTest, ctx: TestContext): Promise<TestResult> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      if (isRetriable) {
        console.log(
          `  RETRY ${test.name} (attempt ${attempt + 1}/${maxAttempts})`,
        );
        continue;
      }
...
function report(results: TestResult[]): number {
  ...
  console.log(
    `Total: ${pass} passed, ${fail} failed, ${results.length} total${retriedSuffix}`,
  );
  return fail;
}
...
async function teardown(ctx: TestContext, failCount: number): Promise<never> {
  console.log("\n=== Teardown ===");
