...
test.describe('Community Lineup banner on Games page', () => {
    ...
    /**
     * ROK-1401 — the Co-Optimus co-op pill on a banner thumbnail.
     *
     * Fixture: the DEMO_MODE `POST /admin/test/seed-cooptimus` endpoint
     * (ROK-1398) returns an enriched game carrying a positive, Co-Optimus-
     * verified `cooptimus_online_max`. We nominate it into whichever lineup
     * the banner is currently serving (that is what
     * `ensureActiveLineupInBuildingPhase` returns), so a sibling worker
     * owning the banner cannot desync the assertion.
     *
     * The expected count is read back off the live banner payload rather than
     * hardcoded, and the banner endpoint is polled BEFORE the UI is touched —
     * React Query's staleTime otherwise serves a pre-nomination fetch
     * (ROK-1156). The Co-Optimus HTTP user-agent is never referenced here.
     */
    test('banner thumbnail shows the co-op pill for an enriched nomination (ROK-1401)', async ({
        page,
    }) => {
        ...
        const bannerLineupId =
            await ensureActiveLineupInBuildingPhase(adminToken);
        // Non-throwing by design: a repeat run (or the sibling project) may
        // have already nominated this game — the poll below is the real gate.
        await apiPost(adminToken, `/lineups/${bannerLineupId}/nominate`, {
            gameId: seed.enrichedGameId,
        });

        const onlineMax = await pollForCondition(
            async () => {
                const b = await apiGet(adminToken, '/lineups/banner');
                const entry = (
                    b?.entries as
                        | { gameId: number; cooptimusOnlineMax?: number | null }[]
                        | undefined
                )?.find((e) => e.gameId === seed.enrichedGameId);
                return typeof entry?.cooptimusOnlineMax === 'number' &&
                    entry.cooptimusOnlineMax > 0
                    ? entry.cooptimusOnlineMax
                    : null;
            },
            {
                timeoutMs: 15_000,
                description:
                    'banner entry exposes a positive cooptimusOnlineMax',
            },
        );
        ...
        const pill = page.getByTestId('coop-pill').first();
        await expect(pill).toBeVisible({ timeout: 15_000 });
        await expect(pill).toHaveText(new RegExp(`${onlineMax} co-op`));
    });
});
