...
| `lfg-group-page.smoke.spec.ts` | Self-contained | None — clears its own intents via `DELETE /lfg/:gameId` for both users in `beforeAll`/`afterAll` | n/a | ROK-1464: picks its fixture game out of `/games/discover` (first row with a slug). Writes LFG intents only; the `Find a time` leg creates a standalone scheduling poll whose lineup title is server-generated, so it is NOT prefix-resettable. |
