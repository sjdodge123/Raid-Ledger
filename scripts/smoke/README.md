...
| `lfg-group-page.smoke.spec.ts` | Self-contained | None — clears its own intents via `DELETE /lfg/:gameId` for both users in `beforeAll`/`afterAll` | n/a | ROK-1464: picks its fixture game out of `/games/discover` by PROJECT index (desktop 0, mobile 1) because LFG state is keyed by (user, game) and both projects share admin + invitee; skips when the catalogue has fewer slugged games than projects. Writes LFG intents; the `Start a scheduling poll` leg creates a standalone scheduling poll whose lineup title is server-generated, so it is NOT prefix-resettable. ROK-1573's Lock-in case writes both users' game-time templates via `PUT /users/me/game-time` (admin: the `scheduling-poll` slot set; invitee: the whole week, reset to empty after) and deletes the event it creates. |
