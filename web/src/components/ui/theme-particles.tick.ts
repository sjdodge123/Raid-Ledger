...
export function tick(
    ...
    if (cfg.bgEffect === 'aurora') drawAurora(ctx, w, h, state.time);
    if (cfg.bgEffect === 'lava') drawLavaGlow(ctx, w, h, state.time);
    if (cfg.bgEffect === 'sun') drawSun(ctx, w, h, state.time);

    for (const p of particles) {
        if (cfg.behavior === 'firefly') {
            tickFirefly(ctx, p, cfg, w, h);
            continue;
        }
        if (cfg.behavior === 'panel-edge') {
            tickPanelEdge(ctx, p, cfg, h, rectCache, state.panelElements);
            continue;
        }
...
function tickDefault(
    ...
    if (oob) { resetParticle(p, cfg, w, h); return; }
