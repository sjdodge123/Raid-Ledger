...
function PullArrow({ progress }: { progress: number }) {
    return (
        <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-xs text-muted">{progress >= 1 ? 'Release to refresh' : 'Pull to refresh'}</span>
        </div>
    );
}

function usePullTouch(onRefresh: () => Promise<void>) {
...
export function PullToRefresh({ onRefresh, children }: PullToRefreshProps) {
    const { pullDistance, isRefreshing, handleTouchStart, handleTouchMove, handleTouchEnd } = usePullTouch(onRefresh);
    ...
    return (
        <div onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd}>
            ...
            >
                {isRefreshing ? <RefreshSpinner /> : <PullArrow progress={progress} />}
            </div>
            {children}
        </div>
    );
