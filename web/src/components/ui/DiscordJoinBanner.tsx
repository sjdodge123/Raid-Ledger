...
function shouldHideBanner(
    pathname: string, isAuthenticated: boolean,
    user: ReturnType<typeof useAuth>['user'],
    data: ReturnType<typeof useDiscordMembership>['data'],
    dismissed: boolean,
) {
    if (isSuppressedRoute(pathname)) return true;
    if (!isAuthenticated || !user) return true;
    if (!user.discordId) return true;
    if (!data?.botConnected) return true;
    if (data.isMember) return true;
    return dismissed;
}
...
export function DiscordJoinBanner() {
    ...
    if (shouldHideBanner(pathname, isAuthenticated, user, data, dismissed)) return null;
    ...
    return (
        <div className="flex items-center justify-between gap-3 bg-[#5865F2] px-4 py-2.5 text-sm text-white shadow-md">
            ...
            <BannerActions inviteUrl={data!.inviteUrl} onDismiss={handleDismiss} />
        </div>
    );
}
