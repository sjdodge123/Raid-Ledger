interface AuroraBand {
    yCenter: number; ySpread: number; color: string; speed: number; freq: number; phase: number;
}

function drawAuroraBand(ctx: CanvasRenderingContext2D, w: number, t: number, band: AuroraBand) {
...
export function drawAurora(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    ...
    ctx.save();
    for (const band of bands) drawAuroraBand(ctx, w, t, band);
    ctx.restore();
}
...
export function drawSun(ctx: CanvasRenderingContext2D, w: number, h: number, t: number) {
    ...
    ctx.save();
    drawRadialLayer(ctx, sx, sy, base * 5 * pulse, [
        [0, 'rgba(255, 200, 60, 0.14)'], [0.25, 'rgba(255, 160, 40, 0.09)'],
        [0.55, 'rgba(240, 110, 20, 0.04)'], [1, 'transparent'],
    ]);
    drawRadialLayer(ctx, sx, sy, base * 2.2 * pulse, [
        [0, 'rgba(255, 220, 100, 0.30)'], [0.5, 'rgba(255, 170, 50, 0.18)'], [1, 'transparent'],
    ]);
    drawRadialLayer(ctx, sx, sy, base * pulse, [
        [0, 'rgba(255, 240, 180, 0.55)'], [0.5, 'rgba(255, 200, 80, 0.40)'], [1, 'transparent'],
    ]);
    ctx.restore();
}
