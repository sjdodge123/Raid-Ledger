...
function buildChartData(summary: EventAttendanceSummaryDto) {
    return [
...
function DonutChart({ chartData, ratePercent }: { chartData: { name: string; value: number; color: string }[]; ratePercent: number }) {
    return (
...
function StatsGrid({ summary }: { summary: EventAttendanceSummaryDto }) {
    return (
        <div className="grid grid-cols-2 gap-3 text-sm">
            <StatPill label="Attended" value={summary.attended} color="bg-emerald-400" />
            <StatPill label="No-Show" value={summary.noShow} color="bg-red-400" />
            <StatPill label="Excused" value={summary.excused} color="bg-amber-400" />
            <StatPill label="Unmarked" value={summary.unmarked} color="bg-gray-500" />
            <div className="col-span-2 pt-2 border-t border-edge">
                <span className="text-muted">Total: {summary.total} signups</span>
            </div>
        </div>
    );
}
    ...
    return (
        <div className="bg-surface rounded-lg border border-edge p-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6">
                <DonutChart chartData={chartData} ratePercent={ratePercent} />
