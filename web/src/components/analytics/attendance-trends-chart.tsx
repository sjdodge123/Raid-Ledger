...
function PeriodToggle({ period, setPeriod }: { period: AttendanceTrendsPeriod; setPeriod: (p: AttendanceTrendsPeriod) => void }) {
    return (
        <div className="flex gap-1 bg-panel rounded-lg p-1">
            {(['30d', '90d'] as const).map((p) => (
                <button
                    key={p}
                    onClick={() => setPeriod(p)}
                    className={`px-3 py-1 text-sm rounded-md transition-colors ${period === p ? 'bg-emerald-600 text-white' : 'text-muted hover:text-foreground'}`}
                >
...
function SummaryStats({ summary }: { summary: { avgAttendanceRate: number; avgNoShowRate: number; totalEvents: number } }) {
    return (
        <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            <div className="text-center">
                ...
                <p className="text-xl font-bold text-foreground">{summary.totalEvents}</p>
            </div>
        </div>
    );
}

function TrendsLineChart({ dataPoints }: { dataPoints: { date: string; attended: number; noShow: number; excused: number }[] }) {
    return (
        <ResponsiveContainer width="100%" height={280}>
            <LineChart data={dataPoints}>
                <CartesianGrid strokeDasharray="3 3" stroke="#374151" />
                <XAxis dataKey="date" stroke="#9CA3AF" fontSize={12} tickFormatter={formatDateTick} />
                <YAxis stroke="#9CA3AF" fontSize={12} />
                <Tooltip contentStyle={TOOLTIP_STYLE} />
                <Legend />
                <Line type="monotone" dataKey="attended" stroke="#34D399" strokeWidth={2} dot={false} name="Attended" />
                <Line type="monotone" dataKey="noShow" stroke="#F87171" strokeWidth={2} dot={false} name="No-Show" />
                <Line type="monotone" dataKey="excused" stroke="#FBBF24" strokeWidth={2} dot={false} name="Excused" />
            </LineChart>
        </ResponsiveContainer>
    );
}
    ...
    return (
        <div className="bg-surface rounded-lg border border-edge p-6">
            ...
            {isLoading ? (
                ...
                <div className="h-64 flex items-center justify-center"><p className="text-muted">No attendance data for this period.</p></div>
            ) : (
                <>
                    <SummaryStats summary={data.summary} />
                    <TrendsLineChart dataPoints={data.dataPoints} />
                </>
            )}
        </div>
    );
