...
/** Build integration nav items from active plugins, excluding Discord (which has its own section). */
export function buildPluginIntegrationItems(plugins: PluginInfoDto[]): NavItem[] {
    const items: NavItem[] = [];
    for (const plugin of plugins) {
        if (plugin.status !== 'active') continue;
        if (plugin.slug === 'discord') continue;
...
/**
 * Build the full sections list (ROK-359 consolidated, ROK-430 Discord section).
 *
 * When discordItems is provided (non-null), a Discord section is inserted
