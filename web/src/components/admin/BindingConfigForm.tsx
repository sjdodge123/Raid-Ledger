...
const PURPOSE_OPTIONS: Record<ChannelType, { value: BindingPurpose; label: string }[]> = {
    voice: [
...
function configForPurpose(purpose: BindingPurpose, v: ConfigValues): UpdateChannelBindingDto['config'] {
    if (purpose === 'game-announcements') return {};
    return {
        minPlayers: v.minPlayers,
        autoClose: v.autoClose,
        gracePeriod: v.gracePeriod,
        ...(purpose === 'general-lobby' && { allowJustChatting: v.allowJustChatting }),
    };
}
...
function PurposeSelect({ id, channelType, value, onChange }: {
    id: string; channelType: ChannelType; value: BindingPurpose; onChange: (v: BindingPurpose) => void;
}) {
    return (
        <div>
            <label htmlFor={id} className="block text-xs text-muted mb-1">Purpose</label>
            <select id={id} value={value} onChange={(e) => onChange(e.target.value as BindingPurpose)}
                className="w-full px-3 py-2 bg-panel border border-edge rounded-lg text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/40">
                {PURPOSE_OPTIONS[channelType].map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
            <p className="text-[11px] text-muted mt-1">
                Voice + a game = Activity Monitor · Voice, no game = General Lobby · Text = Announcements.
                The form won&apos;t let you save an invalid combination.
            </p>
        </div>
    );
}
...
function GameField({ id, value, onChange, autoFocus, violationMessage }: {
    id: string; value: IgdbGameDto | null; onChange: (g: IgdbGameDto | null) => void;
    autoFocus: boolean; violationMessage?: string;
}) {
    return (
        <div>
            <GameSearchInput id={id} value={value} onChange={onChange} autoFocus={autoFocus} error={violationMessage} />
        </div>
    );
}

function useBindingConfigForm(binding: ChannelBindingDto) {
    const [purpose, setPurpose] = useState<BindingPurpose>(binding.bindingPurpose);
    const [game, setGame] = useState<IgdbGameDto | null>(() => toInitialGame(binding));
    ...
    const buildDto = (finalPurpose: BindingPurpose): UpdateChannelBindingDto => {
        const dto: UpdateChannelBindingDto = {
            config: configForPurpose(finalPurpose, { minPlayers, autoClose, gracePeriod, allowJustChatting }),
        };
        if (finalPurpose !== binding.bindingPurpose) dto.bindingPurpose = finalPurpose;
        if (gameId !== (binding.gameId ?? null)) dto.gameId = gameId;
        return dto;
    };

    return {
        purpose, setPurpose, game, handleGameChange, gameRequired,
        violation, channelType, gameId, buildDto,
        minPlayers, setMinPlayers, autoClose, setAutoClose,
        gracePeriod, setGracePeriod, allowJustChatting, setAllowJustChatting,
    };
}

function FormActions({ saveDisabled, isSaving, onCancel }: { saveDisabled: boolean; isSaving: boolean; onCancel: () => void }) {
    return (
        <div className="flex gap-2 pt-2">
            <button type="submit" disabled={saveDisabled} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white rounded-lg text-sm transition-colors">
...
export function BindingConfigForm({ binding, onSave, onCancel, isSaving, saveError }: BindingConfigFormProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => { e.preventDefault(); onSave(binding.id, f.buildDto(f.purpose)); };
    const handleConvert = () => { f.setPurpose('general-lobby'); onSave(binding.id, f.buildDto('general-lobby')); };
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 p-4 bg-overlay/30 rounded-lg border border-edge">
            ...
            <PurposeSelect id={purposeId} channelType={f.channelType} value={f.purpose} onChange={f.setPurpose} />
            {showGameField && (
                <GameField id={gameFieldId} value={f.game} onChange={f.handleGameChange}
                    autoFocus={f.gameRequired} violationMessage={f.gameRequired ? f.violation?.message : undefined} />
            )}
            {f.purpose === 'general-lobby' && <GeneralLobbySection allowJustChatting={f.allowJustChatting} onChange={f.setAllowJustChatting} />}
            {showVoiceFields && <VoiceMonitorFields minPlayers={f.minPlayers} onMinPlayersChange={f.setMinPlayers} autoClose={f.autoClose} onAutoCloseChange={f.setAutoClose} gracePeriod={f.gracePeriod} onGracePeriodChange={f.setGracePeriod} />}
            {f.purpose === 'game-announcements' && <p className="text-sm text-muted">No additional configuration needed for announcement channels.</p>}
            {saveError && <p className="text-sm text-red-400" role="alert">{saveError}</p>}
            <FormActions saveDisabled={saveDisabled} isSaving={isSaving} onCancel={onCancel} />
        </form>
    );
}
