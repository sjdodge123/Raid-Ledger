    ...
    return (
        <div className="bg-warning/5 border border-warning/20 rounded-lg p-4">
            <div className="flex items-start gap-3">
                ...
                <div>
                    <p className="text-sm text-warning font-medium">Password Recovery</p>
                    <p className="text-xs text-warning mt-1">
                        If you forget your password, the only way to recover access is by setting the{' '}
                        <code className="px-1 py-0.5 bg-warning/10 rounded text-warning font-mono text-[11px]">RESET_PASSWORD=true</code>{' '}
...
/** One "Show passwords" state shared by all three fields (ruling 2): any eye toggle or the Checkbox flips it. */
interface Reveal { revealed: boolean; onRevealedChange: (next: boolean) => void }
...
function SecretField({ label, placeholder, autoComplete, value, onChange, reveal, error, children }: SecretFieldProps): JSX.Element {
    return (
        <Field label={label} error={error} className="sm:max-w-md">
            <PasswordInput label={label} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder}
                autoComplete={autoComplete} revealed={reveal.revealed} onRevealedChange={reveal.onRevealedChange} />
            {children}
        </Field>
    );
}
...
function usePasswordForm() {
    ...
    const mismatchError = confirm.length > 0 && !match ? 'Passwords do not match' : undefined;
    const reveal: Reveal = { revealed: show, onRevealedChange: setShow };
    return { current, setCurrent, newPw, setNewPw, confirm, setConfirm, show, setShow, strength, match, mismatchError, reveal };
}
...
function PasswordChangeForm({ onSubmit, isPending }: { onSubmit: (current: string, newPw: string) => void; isPending: boolean }) {
    ...
    const canSubmit = f.current.length > 0 && f.newPw.length >= 8 && f.match;

    return (
        <div className="space-y-3">
            <SecretField label="Current Password" placeholder="Enter current password" autoComplete="current-password"
                value={f.current} onChange={f.setCurrent} reveal={f.reveal} />
            <SecretField label="New Password" placeholder="At least 8 characters" autoComplete="new-password"
                value={f.newPw} onChange={f.setNewPw} reveal={f.reveal}>
                {f.newPw.length > 0 && <StrengthBar strength={f.strength} />}
            </SecretField>
            <SecretField label="Confirm New Password" placeholder="Re-enter new password" autoComplete="new-password"
                value={f.confirm} onChange={f.setConfirm} reveal={f.reveal} error={f.mismatchError} />
            <Checkbox label="Show passwords" checked={f.show} onChange={(e) => f.setShow(e.target.checked)} />
            <Button onClick={() => onSubmit(f.current, f.newPw)} disabled={!canSubmit} loading={isPending} loadingLabel="Changing...">
                Change Password
            </Button>
        </div>
    );
...
/** Discord's brand fill — brand data for Button `brandColor` only (ruling 3), never a theme colour. */
