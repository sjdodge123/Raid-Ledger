...
describe("BindingConfigForm — general rendering", () => {
  ...
  it("renders the channel name in the form heading", () => {
    ...
    expect(
      screen.getByText(/Edit Config: #raid-announcements/),
    ).toBeInTheDocument();
  });
...
describe("BindingConfigForm — form submission", () => {
  ...
  it("Codex P2 pin: saving as General Lobby clears the game explicitly (gameId: null)", () => {
    renderBindingForm({
      id: "uuid-1",
      channelType: "voice",
      bindingPurpose: "game-voice-monitor",
    ...
    fireEvent.change(screen.getByRole("combobox", { name: /purpose/i }), {
      target: { value: "general-lobby" },
    });
    fireEvent.click(screen.getByRole("button", { name: "Save" }));
    const [, dto] = onSave.mock.calls[0] as [
      string,
      { bindingPurpose?: string; gameId?: number | null },
    ];
    expect(dto.bindingPurpose).toBe("general-lobby");
    expect(dto.gameId).toBeNull(); // omitted would silently keep an uneditable game
  });
...
describe("BindingConfigForm — ROK-1416 purpose selector (channelType-filtered)", () => {
  ...
  it("offers only voice purposes (Activity Monitor, General Lobby) for a voice channel", () => {
    ...
    expect(
      within(select).queryByRole("option", { name: /announcement/i }),
    ).not.toBeInTheDocument();
  });
  ...
  it("offers only the announcements purpose for a text channel", () => {
    ...
    const select = purposeSelect();
    expect(
      within(select).getByRole("option", { name: /announcement/i }),
    ).toBeInTheDocument();
    expect(
      within(select).queryByRole("option", { name: /activity monitor/i }),
    ).not.toBeInTheDocument();
    expect(
      within(select).queryByRole("option", { name: /general lobby/i }),
    ).not.toBeInTheDocument();
  });
});
  ...
  it("renders the save error above the form actions", () => {
    render(
      <BindingConfigForm
        ...
        } as Parameters<typeof BindingConfigForm>[0] & {
