...
import {
    deriveSubmitKind,
    type SubmitKind,
} from '../../shared/submit-bar/derive-kind';
...
/** ROK-1374: the vote is closed for the whole hold — before AND after a pick. */
function isHoldOpen(tie: TieReadinessResponseDto | null | undefined): boolean {
    return tie?.status === 'awaiting_pick' || tie?.status === 'picked';
}

const VOTE_CLOSED_REASON = 'voting closed — tied, waiting on the pick';
...
function buildHero(
    ...
    used: number,
    max: number,
    tie?: TieReadinessResponseDto | null,
    ...
    if (tie && isHoldOpen(tie)) {
        return {
            badge,
            task: 'Voting closed on a tie.',
            sub: `Waiting on ${tie.pickerName ?? 'the lineup creator'} to pick · ${voters}.`,
            tone: 'waiting',
        };
    }
    if (submitted) {
...
export function VotingComposite(props: VotingCompositeProps): JSX.Element {
    ...
    const hero = buildHero(lineup, submitted, used, max, tie);

    const handleToggle = (gameId: number): void => {
        if (!canParticipate || holdOpen) return;
                ...
                <div className="flex items-center gap-2 mt-2 px-1">
                    ...
                    <div className="ml-auto flex-shrink-0">
                        <StickyHeroSubmitButton
                            ...
                            max={max}
                            disabled={
                                kind === 'empty' || !canParticipate || holdOpen
                            }
                            disabledReason={
                                holdOpen
                                    ? VOTE_CLOSED_REASON
