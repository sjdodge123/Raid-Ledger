...
function getEditModeState(
  ...
  return {
    ...
    game: editEvent.game
      ? ({
          id: editEvent.game.id,
          name: editEvent.game.name,
          slug: editEvent.game.slug,
          coverUrl: editEvent.game.coverUrl,
        } as FormState["game"])
      : null,
    eventTypeId: null,
    ...
    selectedInstances:
      (editEvent.contentInstances as Record<string, unknown>[]) ?? [],
...
export function computeRecurrenceCount(
  frequency: string,
  startDate: string,
  untilDate: string,
): number {
  ...
  while (count < MAX_RECURRENCE_INSTANCES) {
    const next =
      frequency === "monthly"
        ? advanceMonthly(current, originalDay)
        : advanceWeekly(current, frequency);
    if (next > until) break;
    count++;
    current = next;
  }
...
export function buildSubmitDto(
  ...
  if (form.recurrenceFrequency) {
    recurrence = {
      ...
      until: new TZDate(
        `${form.recurrenceUntil}T23:59:59`,
        resolved,
      ).toISOString(),
    };
  }
  return {
    ...
    recurrence,
    contentInstances:
      form.selectedInstances.length > 0 ? form.selectedInstances : undefined,
