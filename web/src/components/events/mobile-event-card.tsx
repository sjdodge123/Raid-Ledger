...
function GameCoverThumb({ event, showPlaceholder, gameCoverUrl, onError }: {
    event: EventResponseDto; showPlaceholder: boolean; gameCoverUrl: string | null; onError: () => void;
}) {
    return (
...
function AvatarStack({ signupAvatars }: { signupAvatars: Array<{ url: string | null }> }) {
    return (
    ...
    return (
        <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
                <span data-testid="mobile-event-relative" className="text-xs text-dim">{relativeTime}</span>
                {matchesGameTime && <GameTimeBadgeMobile />}
            </div>
            <div className="flex items-center gap-1">
                <AvatarStack signupAvatars={signupAvatars} />
                {signupCount > 0 && (
...
export function MobileEventCard({ event, signupCount = 0, onClick, matchesGameTime }: MobileEventCardProps) {
    ...
    const signupAvatars = (event.signupsPreview ?? []).slice(0, 3).map((signup) => resolveAvatar(toAvatarUser(signup)));

    return (
        <button type="button" onClick={onClick} data-testid="mobile-event-card"
            ...
            <div className="flex-1 min-w-0 p-3 flex flex-col justify-between gap-1">
                ...
                <MobileCardFooter relativeTime={relativeTime} matchesGameTime={matchesGameTime} signupAvatars={signupAvatars} signupCount={signupCount} />
            </div>
        </button>
    );
}
