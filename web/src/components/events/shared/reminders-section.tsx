...
function ReminderToggle({ label, sub, value, onChange }: {
    ...
    return (
        <div className="flex items-center justify-between gap-3 py-3 min-h-[44px] sm:min-h-0">
            ...
            <div className="event-detail-autosub-toggle shrink-0">
                <div className="event-detail-autosub-toggle__track" role="switch" aria-checked={value}
                    aria-label={`${label} reminder`} tabIndex={0} onClick={() => onChange(!value)}
                    onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onChange(!value); } }}>
                    <span className={`event-detail-autosub-toggle__option ${value ? 'event-detail-autosub-toggle__option--active' : ''}`}>On</span>
                    <span className={`event-detail-autosub-toggle__option ${!value ? 'event-detail-autosub-toggle__option--active' : ''}`}>Off</span>
                </div>
            </div>
        </div>
    );
...
export function RemindersSection({
    reminder15min, reminder1hour, reminder24hour,
    onReminder15minChange, onReminder1hourChange, onReminder24hourChange,
    ...
    const handlers = { reminder15min: onReminder15minChange, reminder1hour: onReminder1hourChange, reminder24hour: onReminder24hourChange };

    return (
        <>
            <p className="text-xs text-dim -mt-2">{description}</p>
            <div className="bg-panel/50 border border-edge-subtle rounded-lg px-4 divide-y divide-edge-subtle">
                {REMINDER_OPTIONS.map(({ key, label, sub }) => (
                    <ReminderToggle key={key} label={label} sub={sub} value={values[key]} onChange={handlers[key]} />
                ))}
            </div>
        </>
    );
