...
import {
  ...
  SaveTemplateBar,
  FormFooter,
} from "./create-event-form-sections";

function useCreateEventMutation(
  isEditMode: boolean,
  editEventId: number | undefined,
  ...
  return useMutation({
    mutationFn: (dto: CreateEventDto | UpdateEventDto) => {
      ...
      return isEditMode
        ? updateEvent(editEventId!, dto as UpdateEventDto)
        : createEvent(dto as CreateEventDto);
    },
    onSuccess: (event) => {
      toast.success(
        isSeriesEdit
      ...
      if (schedulingMatchId) {
        completeStandalonePoll(
          ...
          toast.warning(
            "Event created, but auto-signup may not have completed. Voters can sign up manually.",
          );
        });
      }
    ...
    onError: (error: Error) => {
      toast.error(
        error.message || `Failed to ${isEditMode ? "update" : "create"} event`,
      );
    },
  });
}
...
function loadTemplateIntoForm(
  ...
  setForm((prev) => ({
    ...
    maxAttendees: config.maxAttendees
      ? String(config.maxAttendees)
      : prev.maxAttendees,
    autoUnbench: config.autoUnbench ?? prev.autoUnbench,
    recurrenceFrequency:
      config.recurrence?.frequency ?? prev.recurrenceFrequency,
...
function useEndTimePreview(
  ...
  return useMemo(() => {
    ...
    return new TZDate(end, resolved).toLocaleTimeString("en-US", {
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
      timeZone: resolved,
...
function submitForm(
  ...
  mutate: (dto: CreateEventDto | UpdateEventDto) => void,
  isEditMode: boolean,
...
function useCreateEventFormState(
  editEvent?: EventResponseDto,
  seriesScope?: SeriesScope,
  initialGame?: EventFormProps["initialGame"],
  ...
  const { count: interestCount, isLoading: interestLoading } = useWantToPlay(
    form.game?.id ?? undefined,
  );
  ...
  const endTimePreview = useEndTimePreview(
    form.startDate,
    form.startTime,
...
export function CreateEventForm({
  ...
  const navigate = useNavigate();
  const s = useCreateEventFormState(
    editEvent,
    seriesScope,
    initialGame,
    initialStartTime,
  ...
  return (
    ...
    >
      <TemplatesBar
        templates={s.tpl.templates}
        onLoad={(c) => loadTemplateIntoForm(c, s.setForm)}
        onDelete={(id) => s.tpl.deleteTemplateMutation.mutate(id)}
      />
      <GameContentSection
        form={s.form}
        setForm={s.setForm}
        ...
        interestLoading={s.interestLoading}
      />
      ...
      <WhenSection
        ...
        isEditMode={isEditMode}
        tzAbbr={s.tzAbbr}
        endTimePreview={s.endTimePreview}
      ...
      <div className="border-t border-edge-subtle" />
      <SaveTemplateBar
        show={s.tpl.showSaveTemplate}
        name={s.tpl.saveTemplateName}
        isPending={s.tpl.createTemplateMutation.isPending}
        onNameChange={s.tpl.setSaveTemplateName}
        onSave={s.tpl.saveTemplate}
        onClose={() => {
          s.tpl.setShowSaveTemplate(false);
          s.tpl.setSaveTemplateName("");
        }}
      />
      <FormFooter
        isEditMode={isEditMode}
        isPending={s.mutation.isPending}
        onShowSaveTemplate={() => s.tpl.setShowSaveTemplate(true)}
...
function GameContentSection({
  ...
  const onEventTypeIdChange = useCallback(
    (id: number | null) => setForm((prev) => ({ ...prev, eventTypeId: id })),
    [setForm],
  );
    ...
    <FormSection title="Game & Content">
      <GameDetailsSection
        ...
        onTitleChange={(title, isAuto) => {
          ...
          if (!isAuto && errors.title)
            setErrors((prev) => ({ ...prev, title: undefined }));
        }}
        onDescriptionChange={(description, isAuto) =>
          setForm((prev) => ({
            ...prev,
            description,
            descriptionIsAutoSuggested: isAuto,
          }))
        }
        onSelectedInstancesChange={(instances) =>
          setForm((prev) => ({ ...prev, selectedInstances: instances }))
        }
        onEventTypeDefaults={(defaults: Partial<SlotState>) =>
