...
function VoteCount({ option }: { option: PollOptionResult }) {
    return (
...
function VoteBar({ option, maxVotes, isWinner }: {
    option: PollOptionResult; maxVotes: number; isWinner?: boolean;
}) {
    ...
    return (
        <div className="space-y-1">
            <div className="flex items-center justify-between text-sm">
                <VoteBarLabel option={option} isWinner={isWinner} />
                <VoteCount option={option} />
            </div>
            <div className="h-2 bg-surface rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all duration-500 ${isWinner ? 'bg-emerald-500' : 'bg-violet-500/70'}`}
                    style={{ width: `${pct}%` }} />
            </div>
            {option.voters.length > 0 && (
                <p className="text-xs text-muted/70 truncate">
                    {option.voters.map((v) => v.username ?? v.discordId).join(', ')}
                </p>
            )}
        </div>
    );
...
function PollLoadingSkeleton() {
    return (
        <div className="space-y-2 animate-pulse">
            {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-8 bg-surface rounded" />)}
        </div>
    );
}
...
export function PollResultsSection({ planId }: { planId: string }) {
    ...
    if (isLoading) return <PollLoadingSkeleton />;

    if (error || !results) {
        return <p className="text-sm text-muted/70">{error ? 'Could not load poll results' : 'No results available'}</p>;
    }

    if (results.pollOptions.length === 0) return <p className="text-sm text-muted/70">No votes yet</p>;

    const maxVotes = Math.max(...results.pollOptions.map((o) => o.registeredVotes), results.noneOption?.registeredVotes ?? 0);
