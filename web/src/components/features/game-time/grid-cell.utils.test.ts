 ...
 * Fill = availability scaled by the fresh share; the hatch retired with the
 * painted heatmap (ROK-1588).
 */
...
describe('computeHeatmapBg with the freshness model (ROK-1560)', () => {
    ...
    it('draws NO fill where nobody is known but a LIGHTER fill where only stale members cover', () => {
...
describe('computeCellStyle (ROK-1588)', () => {
    it('joins the shadows into one boxShadow', () => {
        expect(computeCellStyle(['a', 'b'])).toEqual({ boxShadow: 'a, b' });
    });

    it('is undefined when there is nothing to style', () => {
        expect(computeCellStyle([])).toBeUndefined();
    });
});
