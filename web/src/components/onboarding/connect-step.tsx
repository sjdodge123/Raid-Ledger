 ...
 */
const CONNECT_ICON_PATH = 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1';
        ...
        <div className="text-center">
            <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-emerald-500/20 flex items-center justify-center">
                <svg className="w-8 h-8 text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={CONNECT_ICON_PATH} />
                </svg>
            </div>
            <h2 className="text-2xl font-bold text-foreground">Connect Your Account</h2>
            <p className="text-muted mt-2">Link a gaming account to sync your profile, avatar, and friends.</p>
        </div>
    );
}

function DiscordConnectButton({ isRedirecting, onClick }: { isRedirecting: boolean; onClick: () => void }) {
    return (
        <button onClick={onClick} disabled={isRedirecting}
            ...
            {isRedirecting ? (
                <><span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Redirecting to Discord...</>
            ) : (
                <><DiscordIcon className="w-5 h-5" />Connect Discord</>
            )}
        </button>
    );
}
