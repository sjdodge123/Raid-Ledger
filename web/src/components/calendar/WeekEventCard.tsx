 ...
 */
function getAvatarConfig(tier: Tier) {
    if (tier === 'compact') return { size: 'xs' as const, max: 3 };
    if (tier === 'standard') return { size: 'sm' as const, max: 5 };
    return null;
}
...
function SignupBadge({ count }: { count: number }) {
    return (
            ...
            <div className="week-event-details">
                ...
                {avatarConfig && signupsPreview && signupsPreview.length > 0 ? (
                    <div className="week-event-attendees">
                        <AttendeeAvatars signups={signupsPreview} totalCount={signupCount} size={avatarConfig.size} maxVisible={avatarConfig.max} accentColor={colors.border} gameId={event.resource?.game?.id ?? undefined} />
                    </div>
                ) : signupCount > 0 ? <SignupBadge count={signupCount} /> : null}
            </div>
        </div>
    );
}
