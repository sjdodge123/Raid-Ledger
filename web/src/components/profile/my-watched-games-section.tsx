 ...
 */
function GameCoverImage({ game }: { game: UserHeartedGameDto }) {
    if (game.coverUrl) {
        return <img src={game.coverUrl} alt={game.name}
...
function HeartIcon({ selected }: { selected: boolean }) {
    return (
        <div className="absolute top-1 left-1 flex items-center justify-center w-11 h-11 rounded-full bg-black/50">
            <svg className={`w-5 h-5 transition-colors ${selected ? 'text-red-400 fill-red-400' : 'text-white/70'}`}
                fill={selected ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
...
function WatchedGameCard({ game, selected, onToggle }: {
    game: UserHeartedGameDto; selected: boolean; onToggle: (gameId: number) => void;
}) {
    return (
        <div role="button" tabIndex={0} onClick={() => onToggle(game.id)}
            ...
            className={`group relative rounded-xl overflow-hidden bg-panel border-2 transition-all cursor-pointer hover:shadow-lg hover:shadow-emerald-900/20 ${
                selected ? 'border-emerald-500 shadow-emerald-500/20 shadow-md' : 'border-edge/50 opacity-50'}`}>
    ...
    return (
        <div className="bg-surface border border-edge-subtle rounded-xl p-6">
            ...
            <div className="text-center py-8">
                ...
                <Link to="/games" className="inline-flex items-center gap-2 text-emerald-400 hover:text-emerald-300 font-medium transition-colors">
...
function SaveBar({ isDirty, isSaving, onReset, onSave }: {
    isDirty: boolean; isSaving: boolean; onReset: () => void; onSave: () => void;
}) {
    if (!isDirty) return null;
    return (
        <div className="flex items-center gap-3">
            <button onClick={onReset} className="text-sm text-muted hover:text-foreground transition-colors">Reset</button>
            <button onClick={onSave} disabled={isSaving}
                ...
                {isSaving ? (<><div className="w-4 h-4 border-2 border-foreground/30 border-t-foreground rounded-full animate-spin" />Saving...</>) : 'Save Changes'}
            </button>
        </div>
    );
}
...
async function removeUnheartedGames(removedIds: number[]) {
    ...
    await Promise.all(removedIds.map(async (gameId) => {
        const res = await fetch(`${API_BASE_URL}/games/${gameId}/want-to-play`, { method: 'DELETE', headers });
        if (!res.ok) throw new Error(`Failed to remove game ${gameId} (${res.status})`);
    }));
}

function useWatchedGamesDirty(localIds: Set<number> | null, serverIds: Set<number>) {
    return useMemo(() => {
        if (!localIds) return false;
        if (localIds.size !== serverIds.size) return true;
        for (const id of localIds) { if (!serverIds.has(id)) return true; }
        return false;
    }, [localIds, serverIds]);
}
...
function useWatchedGamesState() {
    ...
    const handleToggle = useCallback((gameId: number) => {
        setLocalIds((prev) => { const next = new Set(prev ?? new Set(serverIds)); if (next.has(gameId)) next.delete(gameId); else next.add(gameId); return next; });
    }, [serverIds]);
    const handleSave = useCallback(async () => {
        if (!isDirty || isSaving) return;
        setIsSaving(true);
        try {
            await removeUnheartedGames([...serverIds].filter((id) => !effectiveIds.has(id)));
    ...
    return { serverGames, isLoading, effectiveIds, isDirty, isSaving, handleToggle, handleReset: useCallback(() => setLocalIds(null), []), handleSave };
}
    ...
    return (
        <div className="bg-surface border border-edge-subtle rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
                ...
                <SaveBar isDirty={s.isDirty} isSaving={s.isSaving} onReset={s.handleReset} onSave={s.handleSave} />
            </div>
            <p className="text-sm text-muted mb-5">Click a game to toggle your interest. Changes are saved when you click "Save Changes".</p>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {s.serverGames.map((game) => (
                    <WatchedGameCard key={game.id} game={game} selected={s.effectiveIds.has(game.id)} onToggle={s.handleToggle} />
                ))}
            </div>
        </div>
    );
