 ...
 * binary stop/start.
 */
function buildNodeCache(container: HTMLElement): NodeCache[] {
    ...
    els.forEach((el) => {
        ...
        nodes.push({ el, angleRad: (angleDeg * Math.PI) / 180, radius: radiusPx, isSpoke: el.classList.contains('spoke-node') });
    });
    return nodes;
}

function positionNode(node: NodeCache, orbitRad: number) {
    const totalAngle = node.angleRad + orbitRad;
    const x = Math.cos(totalAngle) * node.radius;
    const y = Math.sin(totalAngle) * node.radius * COS_TILT;
...
function updateVelocity(velocityRef: React.MutableRefObject<number>, isHovered: boolean) {
    if (isHovered) {
...
function createAnimationLoop(
    containerRef: React.RefObject<HTMLDivElement | null>, nodeCacheRef: React.MutableRefObject<NodeCache[] | null>,
    currentAngleRef: React.MutableRefObject<number>, velocityRef: React.MutableRefObject<number>,
    lastFrameRef: React.MutableRefObject<number>, isHoveredRef: React.MutableRefObject<boolean>,
    ...
    const animate = () => {
        ...
        updateVelocity(velocityRef, isHoveredRef.current);
        currentAngleRef.current += ((2 * Math.PI) / ORBIT_DURATION) * dt * velocityRef.current;
        for (const node of nodeCacheRef.current) positionNode(node, currentAngleRef.current);
...
export function useOrbitalAnimation(enabled = true) {
    ...
    useEffect(() => {
        ...
        nodeCacheRef.current = buildNodeCache(container);
        nodeCacheRef.current.forEach((n) => positionNode(n, 0));
        lastFrameRef.current = Date.now();
        const animate = createAnimationLoop(containerRef, nodeCacheRef, currentAngleRef, velocityRef, lastFrameRef, isHoveredRef, animFrameRef);
        animFrameRef.current = requestAnimationFrame(animate);
        return () => {
