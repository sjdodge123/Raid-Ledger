...
const NOTIFICATION_TYPE_META: {
  ...
  },
  {
    ...
    description:
      "Updates about community lineup voting, matches, and scheduling",
  },
  {
    type: "post_event_followup",
    label: "Post-Event Follow-Ups",
    description:
      "Prompts to schedule a follow-up and quick sign-up invites after an event ends",
  },
];
      ...
      <div className="flex gap-2 sm:gap-4 shrink-0">
        {visibleChannels.map(({ channel }) => (
          <ChannelToggle
            key={channel}
            channel={channel}
            active={preferences?.channelPrefs[type]?.[channel] ?? false}
            label={label}
            onToggle={() => onToggle(type, channel)}
          />
        ))}
      </div>
    </div>
...
function NotificationPreferencesContent({
  ...
  channelAvailability: ReturnType<
  ...
  return (
    <div>
      ...
      {channelAvailability?.discord &&
        !channelAvailability.discord.available &&
