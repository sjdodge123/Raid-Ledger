...
function GuestBadge({ canManage, inviteUrl, isAnonymous, onCopy }: {
    canManage: boolean; inviteUrl: string | null; isAnonymous: boolean; onCopy: (e: React.MouseEvent) => void;
}) {
    if (canManage && inviteUrl) {
        return (
            <button onClick={onCopy} className="shrink-0 inline-flex items-center gap-1 rounded-full bg-blue-600/30 px-2 py-0.5 text-xs font-medium text-blue-300 hover:bg-blue-600/50 transition-colors cursor-pointer" title="Click to copy invite link">
    ...
    return <span className="shrink-0 rounded-full bg-amber-600/30 px-2 py-0.5 text-xs font-medium text-amber-300">{isAnonymous ? 'Invite' : 'Guest'}</span>;
}

function ServerInviteLink({ url }: { url: string }) {
    return (
...
function PugCardInfo({ pug, statusInfo, canManage, inviteUrl, showRole, onCopy }: {
    pug: PugSlotResponseDto; statusInfo: { dot: string; label: string };
    canManage: boolean; inviteUrl: string | null; showRole: boolean; onCopy: (e: React.MouseEvent) => void;
}) {
    const isAnonymous = !pug.discordUsername;
    return (
        <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
                <span className={`truncate font-medium ${isAnonymous ? 'text-muted italic' : 'text-foreground'}`} title={pug.discordUsername ?? 'Awaiting player'}>
                    {pug.discordUsername ?? 'Awaiting player'}
                </span>
                <GuestBadge canManage={canManage} inviteUrl={inviteUrl} isAnonymous={isAnonymous} onCopy={onCopy} />
                {showRole && (
                    <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium whitespace-nowrap ${ROLE_BADGE_CLASSES[pug.role] ?? ''}`}>
                        <RoleIcon role={pug.role} size="w-3.5 h-3.5" /> {formatRole(pug.role)}
                    </span>
                )}
            </div>
            <div className="flex items-center gap-2 text-xs text-muted">
                {(pug.class || pug.spec) && <span className="truncate">{[pug.class, pug.spec].filter(Boolean).join(' \u2022 ')}</span>}
                <span className="flex items-center gap-1 shrink-0">
                    <span className={`inline-block h-2 w-2 rounded-full ${statusInfo.dot}`} /><span>{statusInfo.label}</span>
                </span>
            </div>
            {pug.notes && <p className="mt-0.5 truncate text-xs text-dim" title={pug.notes}>{pug.notes}</p>}
            {canManage && pug.serverInviteUrl && pug.status === 'pending' && !inviteUrl && <ServerInviteLink url={pug.serverInviteUrl} />}
        </div>
    );
}
    ...
    return (
        <div className="relative shrink-0">
            <button onClick={(e) => { e.stopPropagation(); setShowMenu(!showMenu); }}
                className="flex items-center justify-center w-11 h-11 rounded text-dim hover:bg-panel hover:text-foreground transition-colors" aria-label="PUG actions">
...
export function PugCard({ pug, canManage = false, onEdit, onRemove, onRegenerateLink, showRole = false }: PugCardProps) {
    ...
    return (
        <div className="relative flex items-center gap-3 rounded-lg border border-dashed border-amber-500/40 bg-amber-900/10 p-2 min-h-[60px] transition-all">
            <PugAvatar username={pug.discordUsername ?? null} discordUserId={pug.discordUserId} discordAvatarHash={pug.discordAvatarHash} sizeClassName="h-8 w-8" />
            <PugCardInfo pug={pug} statusInfo={statusInfo} canManage={canManage} inviteUrl={inviteUrl} showRole={showRole} onCopy={handleCopyInviteUrl} />
            {canManage && <PugCardMenu pug={pug} inviteUrl={inviteUrl} onEdit={onEdit} onRemove={onRemove} onRegenerateLink={onRegenerateLink} onCopy={handleCopyInviteUrl} />}
        </div>
    );
}
