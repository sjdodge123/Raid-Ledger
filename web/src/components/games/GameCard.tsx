...
export function GameCard({ game, compact = false }: GameCardProps) {
  ...
  return (
    <Link
      to={`/games/${game.id}`}
      className={`group block relative rounded-xl overflow-hidden bg-panel border border-edge/50 hover:border-emerald-500/50 transition-all hover:shadow-lg hover:shadow-emerald-900/20 ${
        compact ? "w-[180px] flex-shrink-0" : ""
      }`}
    >
      {/* Cover Image */}
      <div className="relative aspect-[3/4] bg-panel">
        ...
        {isAuthenticated && (
          <button
            ...
            aria-label={
              wantToPlay ? "Remove from want to play" : "Add to want to play"
            }
          >
            <svg
              className={`w-5 h-5 transition-colors ${
                wantToPlay ? "text-red-400 fill-red-400" : "text-white/70"
              }`}
