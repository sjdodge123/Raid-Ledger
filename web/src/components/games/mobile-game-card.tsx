...
function MobileHeartButton({ wantToPlay, count, onClick }: { wantToPlay: boolean; count: number; onClick: (e: React.MouseEvent) => void }) {
    return (
        <button onClick={onClick} data-testid="mobile-game-heart" className="absolute top-1 left-1 flex items-center justify-center w-11 h-11 rounded-full bg-black/50 hover:bg-black/70 transition-colors" aria-label={wantToPlay ? 'Remove from want to play' : 'Add to want to play'}>
...
export function MobileGameCard({ game }: MobileGameCardProps) {
    ...
    return (
        <Link to={`/games/${game.id}`} data-testid="mobile-game-card" className="group block relative rounded-lg overflow-hidden bg-surface border border-edge hover:border-dim transition-all">
            <div className="relative aspect-[3/4] bg-panel">
                {game.coverUrl ? <img src={game.coverUrl} alt={game.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" /> : (
                    <div className="w-full h-full flex items-center justify-center text-dim"><svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                )}
                {rating && rating > 0 && <div data-testid="mobile-game-rating" className={`absolute top-2 right-2 px-2 py-0.5 rounded-md text-xs font-bold ${getRatingClasses(rating)}`}>{Math.round(rating)}</div>}
                {isAuthenticated && <MobileHeartButton wantToPlay={wantToPlay} count={count} onClick={handleWantToPlay} />}
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2 pt-6">
                    <h3 className="text-sm font-semibold text-white line-clamp-2 leading-tight">{game.name}</h3>
                    {primaryGenre && <span data-testid="mobile-game-genre" className="inline-block mt-0.5 px-1.5 py-0.5 text-[10px] bg-white/20 text-white/90 rounded">{primaryGenre}</span>}
                </div>
            </div>
        </Link>
    );
