    ...
    return (
        <Modal isOpen={isOpen} onClose={() => { onClose(); setFilterSearch(''); }} title="Filter by Game" maxWidth="max-w-sm" initialFocusRef={searchInputRef}>
            ...
            <input type="text" value={filterSearch} onChange={(e) => setFilterSearch(e.target.value)} placeholder="Search games..."
                className="w-full px-3 py-2 mb-3 rounded-lg bg-panel border border-edge text-base lg:text-sm text-foreground placeholder:text-muted focus:outline-none focus:ring-2 focus:ring-success/50 focus:border-transparent transition-colors"
                ref={searchInputRef} />
