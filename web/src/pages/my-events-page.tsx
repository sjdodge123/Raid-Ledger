...
function MyEventsEmptyState({ activeTab, isAdmin, onTabChange }: { activeTab: Tab; isAdmin: boolean; onTabChange: (t: Tab) => void }) {
    return (
        <div className="py-8 px-4">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="text-center py-16">
                    <p className="text-lg text-muted mb-4">{isAdmin ? 'No upcoming events yet.' : "You don't have any event metrics yet."}</p>
...
export function MyEventsPage() {
    ...
    if (dashboard.error) return <MyEventsError message={dashboard.error.message} />;
    if (activeTab === 'dashboard' && !dashboard.isLoading && (!dashboard.data || dashboard.data.events.length === 0)) {
        return <MyEventsEmptyState activeTab={activeTab} isAdmin={isAdmin} onTabChange={setActiveTab} />;
    }
...
function DashboardStatsSection({ dashboard, handleNeedsAttentionClick }: { dashboard: ReturnType<typeof useMyDashboard>; handleNeedsAttentionClick: () => void }) {
    if (dashboard.isLoading) return <DashboardStatsRowSkeleton />;
    if (dashboard.data) return <DashboardStatsRow stats={dashboard.data.stats} onNeedsAttentionClick={handleNeedsAttentionClick} />;
    return null;
}

function EventCardsGrid({ dashboard, highlightGaps, eventsGridRef, isAdmin }: {
    dashboard: ReturnType<typeof useMyDashboard>; highlightGaps: boolean; eventsGridRef: React.RefObject<HTMLDivElement | null>; isAdmin: boolean;
}) {
    return (
        <div ref={eventsGridRef}>
            <h2 className="text-xl font-semibold text-foreground mb-4">{isAdmin ? 'All Upcoming Events' : 'Your Events'}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {dashboard.isLoading
                    ? Array.from({ length: 3 }).map((_, i) => <DashboardEventCardSkeleton key={i} />)
                    : dashboard.data?.events.map((event) => <DashboardEventCard key={event.id} event={event} highlighted={highlightGaps && event.missingRoles.length > 0} />)}
            </div>
        </div>
    );
}

function DashboardTab({ dashboard, highlightGaps, eventsGridRef, handleNeedsAttentionClick, isAdmin }: {
    dashboard: ReturnType<typeof useMyDashboard>; highlightGaps: boolean; eventsGridRef: React.RefObject<HTMLDivElement | null>;
    handleNeedsAttentionClick: () => void; isAdmin: boolean;
}) {
    return (
        <div className="space-y-8">
            <DashboardStatsSection dashboard={dashboard} handleNeedsAttentionClick={handleNeedsAttentionClick} />
            <EventCardsGrid dashboard={dashboard} highlightGaps={highlightGaps} eventsGridRef={eventsGridRef} isAdmin={isAdmin} />
            <div>
                <h2 className="text-xl font-semibold text-foreground mb-4">Recent Activity</h2>
                <div className="bg-surface rounded-lg border border-edge p-4"><ActivityFeed /></div>
            </div>
        </div>
    );
}
