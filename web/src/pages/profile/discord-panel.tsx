...
function DiscordLinkButton({ onLink }: { onLink: () => void }) {
    return (
        <button onClick={onLink} className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#5865F2] hover:bg-[#4752C4] text-white font-medium rounded-lg transition-colors">
...
export function ProfileDiscordPanel() {
    ...
    if (!user) return null;
    if (systemStatus && !systemStatus.discordConfigured) return <Navigate to="/profile/identity" replace />;
        ...
        <div className="space-y-6">
            <div className="bg-surface border border-edge-subtle rounded-xl p-6">
                ...
                <p className="text-sm text-muted mb-5">
                    {hasDiscordLinked ? 'Your Discord account is linked. This enables rich notifications and authentication.' : 'Link your Discord account for authentication and notifications.'}
                </p>
                {hasDiscordLinked ? <DiscordLinkedState user={user} /> : <DiscordLinkButton onLink={handleLinkDiscord} />}
            </div>
        </div>
    );
}
