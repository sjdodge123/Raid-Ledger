        ...
        <div className="mt-4 p-4 bg-panel rounded-lg border border-edge">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2" data-testid="steam-action-buttons">
                    <button onClick={() => syncLibrary.mutate()} disabled={syncLibrary.isPending || unlinkSteam.isPending} className="text-sm text-success hover:text-success/80 disabled:opacity-50">
                        {syncLibrary.isPending ? 'Syncing...' : 'Sync Library'}
                    </button>
                    <span className="text-muted">|</span>
                    <button onClick={() => syncWishlist.mutate()} disabled={syncWishlist.isPending || unlinkSteam.isPending} className="text-sm text-success hover:text-success/80 disabled:opacity-50">
