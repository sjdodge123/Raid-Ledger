...
/** Icon component for theme mode (sun, moon, monitor) */
function ModeIcon({ icon, className }: { icon: 'sun' | 'moon' | 'monitor'; className?: string }): JSX.Element {
...
/** Visual cards for available themes */
function ThemePicker({ label, themes, activeId, onSelect }: {
    label?: string; themes: ThemeDefinition[]; activeId: string; onSelect: (id: string) => void;
}): JSX.Element | null {
    ...
    return (
        <button onClick={onClick}
            className={`relative flex flex-col items-start gap-2 p-3 rounded-lg border-2 transition-colors text-left ${
                isActive ? 'border-emerald-500 bg-emerald-500/10' : 'border-edge bg-panel hover:border-edge-strong'
            }`}>
