import { useAdminSettings } from '../../hooks/use-admin-settings';
import { IntegrationCard } from '../../components/admin/IntegrationCard';
import { IgdbForm } from '../../components/admin/IgdbForm';

const TwitchIcon = (
    <div className="w-10 h-10 rounded-lg bg-[#9146FF] flex items-center justify-center">
        <svg className="w-6 h-6 text-foreground" viewBox="0 0 24 24" fill="currentColor">
            <path d="M11.571 4.714h1.715v5.143H11.57zm4.715 0H18v5.143h-1.714zM6 0L1.714 4.286v15.428h5.143V24l4.286-4.286h3.428L22.286 12V0zm14.571 11.143l-3.428 3.428h-3.429l-3 3v-3H6.857V1.714h13.714Z" />
        </svg>
    </div>
);

/**
 * Integrations > IGDB / Twitch panel.
 * ROK-281: Wraps existing IntegrationCard + IgdbForm.
 */
export function IgdbPanel() {
    const { igdbStatus } = useAdminSettings();

    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-xl font-semibold text-foreground">IGDB / Twitch</h2>
                <p className="text-sm text-muted mt-1">Enable game discovery and live stream integration.</p>
            </div>
            <IntegrationCard
                title="IGDB / Twitch"
                description="Enable game discovery and live streams"
                icon={TwitchIcon}
                isConfigured={igdbStatus.data?.configured ?? false}
                isLoading={igdbStatus.isLoading}
                defaultExpanded
            >
                <IgdbForm />
            </IntegrationCard>
        </div>
    );
}
