...
function toastResult(data: { success: boolean; message: string }) {
...
function DiscordOverviewContent() {
    ...
    const handleReconnect = () => { reconnectBot.mutate(undefined, { onSuccess: toastResult, onError: () => toast.error('Failed to reconnect') }); };
    const handleTestMessage = () => { sendTestMessage.mutate(undefined, { onSuccess: toastResult, onError: (err: Error) => toast.error(err.message) }); };

    return (
        <div className="space-y-6">
            <div><h2 className="text-xl font-semibold text-foreground">Discord Overview</h2>
                <p className="text-sm text-muted mt-1">Setup progress and bot status at a glance.</p></div>
            <SetupProgressCard setup={setupStatus.data} isLoading={setupStatus.isLoading} />
            <BotStatusCard botData={botData} />
            <QuickActionsCard onReconnect={handleReconnect} onTestMessage={handleTestMessage}
                isReconnecting={reconnectBot.isPending} isSending={sendTestMessage.isPending}
                isConfigured={botData?.configured ?? false} isConnected={botData?.connected ?? false} />
        </div>
    );
}
...
function SetupProgressCard({ setup, isLoading }: {
    setup: { completedCount: number; totalCount: number; steps: { key: string; label: string; completed: boolean; settingsPath: string }[] } | undefined;
    isLoading: boolean;
}) {
    return (
        <div className="bg-surface border border-edge rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
                ...
                {setup && <span className="text-sm text-muted">{setup.completedCount}/{setup.totalCount} complete</span>}
            </div>
            {setup && (
                <>
            ...
            {isLoading && <div className="animate-pulse space-y-3">{[1, 2, 3, 4, 5].map((i) => <div key={i} className="h-6 bg-overlay rounded" />)}</div>}
        </div>
    );
}
                ...
                <div className="flex-1">
                    ...
                    {botData?.connected && botData.guildName && (
                        <p className="text-xs text-secondary mt-0.5">
                            {botData.guildName}
                            {botData.memberCount != null && <span className="text-dim"> &middot; {botData.memberCount} members</span>}
                        </p>
                    )}
                </div>
            </div>
...
function QuickActionsCard({ onReconnect, onTestMessage, isReconnecting, isSending, isConfigured, isConnected }: {
    onReconnect: () => void; onTestMessage: () => void;
    isReconnecting: boolean; isSending: boolean; isConfigured: boolean; isConnected: boolean;
}) {
    return (
        <div className="bg-surface border border-edge rounded-xl p-6">
            <h3 className="text-base font-semibold text-foreground mb-4">Quick Actions</h3>
            <div className="flex flex-wrap gap-3">
                <button onClick={onReconnect} disabled={isReconnecting || !isConfigured}
                    ...
                    {isReconnecting ? 'Reconnecting...' : 'Reconnect Bot'}
                </button>
                <button onClick={onTestMessage} disabled={isSending || !isConnected}
