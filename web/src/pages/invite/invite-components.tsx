...
function StepDot({ stepNum, isActive, isCompleted, label }: { stepNum: number; isActive: boolean; isCompleted: boolean; label: string }) {
    ...
    return (
        <div className="flex flex-col items-center">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold transition-colors ${circleClass}`}>
                {isCompleted ? (
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                ) : stepNum}
            </div>
            <span className={`text-[10px] mt-1 ${isActive ? 'text-foreground' : 'text-muted'}`}>{label}</span>
        </div>
    );
}
    ...
    return (
        <>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted truncate">{[character.realm, character.class, character.spec].filter(Boolean).join(' - ')}</div>
            </div>
...
export function CharacterCard({ character, isSelected, onSelect }: { character: CharacterDto; isSelected: boolean; onSelect: () => void }): JSX.Element {
    return (
        <button type="button" onClick={onSelect}
            className={`w-full flex items-center gap-3 p-3 rounded-lg border transition-colors text-left ${isSelected ? 'bg-emerald-600/10 border-emerald-500' : 'bg-panel border-edge hover:border-foreground/30'}`}>
            <CharacterCardContent character={character} isSelected={isSelected} />
        </button>
    );
}
