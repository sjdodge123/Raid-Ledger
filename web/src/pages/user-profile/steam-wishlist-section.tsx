  ...
  return (
    <div className="user-profile-section">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="user-profile-section-title mb-0">Steam Wishlist{total > 0 ? ` (${total})` : ""}</h2>
      </div>
      <div className="flex flex-col gap-2">
        {items.map((entry) => <WishlistCard key={entry.gameId} entry={entry} pricing={pricingMap.get(entry.gameId)} />)}
      </div>
      {total > 10 && (
      ...
      <SteamWishlistModal userId={userId} isOpen={showModal} onClose={() => setShowModal(false)} total={total} pricingMap={pricingMap} />
    </div>
  );
}
