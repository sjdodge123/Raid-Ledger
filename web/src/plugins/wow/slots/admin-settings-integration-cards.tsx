...
const EYE_OFF_PATH = 'M3.98 8.223A10.477 10.477 0 0 0 1.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.451 10.451 0 0 1 12 4.5c4.756 0 8.773 3.162 10.065 7.498a10.522 10.522 0 0 1-4.293 5.774M6.228 6.228 3 3m3.228 3.228 3.65 3.65m7.894 7.894L21 21m-3.228-3.228-3.65-3.65m0 0a3 3 0 1 0-4.243-4.243m4.242 4.242L9.88 9.88';
const EYE_PATHS = [
    ...
    return (
        <button type="button" onClick={onToggle} aria-label={show ? 'Hide password' : 'Show password'}
            ...
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                {show ? <path strokeLinecap="round" strokeLinejoin="round" d={EYE_OFF_PATH} />
                    : EYE_PATHS.map((d) => <path key={d.slice(0, 10)} strokeLinecap="round" strokeLinejoin="round" d={d} />)}
            </svg>
        </button>
    );
}
...
function BlizzardConfigForm({ isConfigured, clientId, clientSecret, showSecret, testResult, onClientIdChange, onClientSecretChange, onToggleSecret, onSave, onTest, onClear, savePending, testPending, clearPending }: {
    isConfigured: boolean; clientId: string; clientSecret: string; showSecret: boolean;
    testResult: { success: boolean; message: string } | null;
    onClientIdChange: (v: string) => void; onClientSecretChange: (v: string) => void;
    onToggleSecret: () => void; onSave: (e: React.FormEvent) => void; onTest: () => void; onClear: () => void;
    ...
    return (
        <form onSubmit={onSave} className="space-y-4">
            <div>
                <label htmlFor="blizzardClientId" className="block text-sm font-medium text-secondary mb-1.5">Client ID</label>
                <input id="blizzardClientId" type="text" value={clientId} onChange={(e) => onClientIdChange(e.target.value)}
                    placeholder={`${placeholder} Client ID`} className="w-full px-4 py-3 bg-surface/50 border border-edge rounded-lg text-foreground placeholder:text-dim focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
            </div>
            <div>
                <label htmlFor="blizzardClientSecret" className="block text-sm font-medium text-secondary mb-1.5">Client Secret</label>
                <div className="relative">
                    <input id="blizzardClientSecret" type={showSecret ? 'text' : 'password'} value={clientSecret} onChange={(e) => onClientSecretChange(e.target.value)}
                        placeholder={`${placeholder} Client Secret`} className="w-full px-4 py-3 pr-12 bg-surface/50 border border-edge rounded-lg text-foreground placeholder:text-dim focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all" />
                    <EyeToggle show={showSecret} onToggle={onToggleSecret} />
                </div>
            </div>
            {testResult && (
                <div className={`p-3 rounded-lg animate-[fadeIn_0.3s_ease-in] ${testResult.success ? 'bg-emerald-500/10 border border-emerald-500/30 text-emerald-400' : 'bg-red-500/10 border border-red-500/30 text-red-400'}`}>{testResult.message}</div>
            )}
...
function BlizzardActionButtons({ isConfigured, savePending, testPending, clearPending, onTest, onClear }: {
    ...
    return (
        <div className="flex flex-wrap gap-3 pt-2">
            <button type="submit" disabled={savePending} className="flex-1 py-3 px-4 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:cursor-not-allowed text-foreground font-semibold rounded-lg transition-colors">
                {savePending ? 'Saving...' : 'Save Configuration'}
            </button>
            {isConfigured && (
                <>
                    <button type="button" onClick={onTest} disabled={testPending} className="py-3 px-4 bg-emerald-600 hover:bg-emerald-500 disabled:bg-emerald-800 disabled:cursor-not-allowed text-foreground font-semibold rounded-lg transition-colors">
                        {testPending ? 'Testing...' : 'Test Connection'}
                    </button>
                    <button type="button" onClick={onClear} disabled={clearPending} className="py-3 px-4 bg-red-600/20 hover:bg-red-600/30 text-red-400 font-semibold rounded-lg transition-colors border border-red-600/50">Clear</button>
                </>
            )}
        </div>
    );
    ...
    return (
        <IntegrationCard title="Blizzard API" description="Enable WoW Armory character import" pluginBadge={getPluginBadge('blizzard')}
            ...
            <BlizzardConfigForm isConfigured={blizzardStatus.data?.configured ?? false} clientId={clientId} clientSecret={clientSecret}
                showSecret={showSecret} testResult={testResult} onClientIdChange={setClientId} onClientSecretChange={setClientSecret}
