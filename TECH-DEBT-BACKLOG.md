...
### 2026-05-22 — rok-1335-recruitment-reminder-stale-channel (surfaced during validate-migrations.sh)

- **[high]** `scripts/validate-migrations.sh` (and any branch that touches `drizzle/migrations/**`) fails because `npx drizzle-kit migrate` exits 1 silently against a fresh Postgres container — confirmed pre-existing on `origin/main` (a861b11d) via worktree-stashed checkout test. The drizzle-kit CLI swallows its own error via its spinner UX (hanji library); no message reaches stdout/stderr even with `NO_COLOR=1 TERM=dumb`, `script -qec`, or `--enable-source-maps`. Likely related to the broken snapshot chain documented in the 2026-05-17 entry above (0140 missing snapshot; 0141 + 0142 both `prevId: eb6febd0…` = 0139's id). Runtime migrations are NOT affected — `api/scripts/run-migrations-with-sentry.ts` uses Drizzle's programmatic migrator and applies cleanly; integration tests on the fleet pass 285/285 against a freshly-migrated DB; raw `psql -v ON_ERROR_STOP=1 < <each-sql>` in journal order applies all 144 migrations cleanly (verified 2026-05-22 on fleet slot 1). Only the standalone `drizzle-kit migrate` CLI is broken.
  Impact: any PR that adds a migration file unmasks this failure in `validate-ci.sh`. GitHub CI's e2e job (`.github/workflows/ci.yml` step `Run database migrations`) ALSO runs `npm run db:migrate -w api` = `drizzle-kit migrate`, so it may be similarly affected — needs verification next time a migration PR ships.
  Suggested: either (a) rewrite `validate-migrations.sh` to apply `*.sql` files via `psql` in journal order (`jq -r '.entries[].tag' meta/_journal.json | xargs -I{} psql -v ON_ERROR_STOP=1 -f migrations/{}.sql`) — bypasses drizzle-kit's broken UX entirely, mirrors what we just verified manually; OR (b) fix the snapshot chain by hand-writing `0140_snapshot.json` (regenerate from 0139's state + 0140's SQL) and fixing the duplicate `prevId` on 0141/0142. Option (a) is small + decoupled from drizzle-kit's upstream behavior; (b) restores drizzle-kit's intended path but is fragile to future drift.
