...
- **[med]** `web/src/pages/event-detail-page.tsx:227` — Navigating to `/events/:id` (e.g., `/events/1`) renders `<EventDetailError message="Event not found" />` even though `GET /events/:id/detail` returns **200 OK with valid data** (verified via curl + `fetch()` in the same browser session). The `useEventDetail` hook (`web/src/hooks/use-events.ts:59-65`) calls `getEventDetail(eventId)` which uses Zod schema parsing (`web/src/lib/api/events-api.ts:81-85`). Suspected cause: Zod schema validation failure that surfaces as a parse error which the page treats as "not found", OR a route param coercion mismatch (page passes `eventId` as `number` but Zod expects something else). **Not introduced by ROK-1260** — discovered during ROK-1260 browser testing of the signup-cancel cascade; the cascade itself works correctly at the DB layer and is verified via `/admin/test/dispatch-discord-notification`. Pre-existing — git blame on `event-detail-page.tsx` shows no changes from ROK-1260's branch touching this code path.
  Suggested: open DevTools, navigate to `/events/1`, look at the actual `detailError.message` text being passed to `EventDetailError` — it'll reveal the Zod issue or the underlying error. If it's a schema mismatch, regenerate types via `npm run build -w packages/contract` and re-check. If it's a route coercion issue, the page reads `useParams<{ id: string }>()` and casts to number — verify the cast doesn't silently produce `NaN` for any edge case.
...
- **[med]** Deactivated users can still LOGIN via OAuth and CREATE new signups on future events. Operator's spec for ROK-1260 explicitly listed "Hiding deactivated users from auth/login flows" as OUT OF SCOPE — they can still log in; we just stop notifying & counting them. AC-5 cancels EXISTING upcoming signups at deactivation time but does NOT prevent the user from re-signing-up afterward (the signup controller has no `deactivated_at IS NULL` check). Architect POST_REVIEW: defensible as-shipped because (a) AC-7/AC-8 hide them from notification fanout so other users can't see them, (b) AC-10 makes their profile 404 to non-admins, (c) they functionally can't affect any list/metric/roster visible to others. But a future "rejoin Discord OR re-bind from web" UX would be a cleaner story.
  Suggested: file a follow-up Linear story (`tech-debt:` or `feat:`) that adds a `deactivated_at IS NULL` gate at the signup controller, the lineup-invitee accept endpoint, and any other "create" endpoint that should refuse deactivated users. Note: Codex flagged this as P1 in `planning-artifacts/review-ROK-1260.md` ("auth bypass") — operator-scope decision was explicit, so this is a re-scoping question, not a ROK-1260 regression.
...
**Real product bugs surfaced by validation:**

- **[med]** `api/src/lineups/lineups-match-response.helpers.ts` `buildGroupedMatchesResponse` — hard-codes `carriedForward: []` in the response. The decided-view `CarriedForwardSection` chip strip (rendered inside `DecidedMatchesView`) reads from `data.carriedForward` and so its chip strip never appears despite the entries table correctly storing `carriedOverFrom`. Building-phase `NominationCard` "Carried Over" label is fine (reads from `entry.carriedOver` on `GET /lineups/:id`); the new `lineup-carryover.smoke.spec.ts` asserts on that healthy path. Silently broken decided-view surface.
  Suggested: populate `result.carriedForward` from `communityLineupEntries WHERE carriedOverFrom IS NOT NULL` for the lineup, mapping each row to the contract's `CarriedForwardEntrySchema`. Add a vitest unit + integration assertion that the chip section is visible whenever any entry has `carriedOver === true`. Visual-only, no data loss.
...
- **[low]** `web/src/components/lineups/NominateModal.tsx` and `api/src/lineups/lineups-nominate.controller.ts` — Steam-unlinked-nominator warning UI (Linear AC: "Nominator without Steam linked → nomination allowed with warning (private lineup flow)") is not exercised by any existing Playwright spec or companion-bot smoke. The nomination modal accepts both name-search and Steam-URL paste; the Steam-unlinked branch behaviour was not driven during ROK-1068 Chrome MCP validation.
  Suggested: add a smoke spec (or extend `lineup-private-dm-only.smoke.spec.ts`) that seeds an invitee user with `steam_id IS NULL` via DEMO_MODE helper, opens the nomination modal as that user, asserts the warning copy renders, and asserts the nomination still POSTs successfully.

- **[low]** `api/src/lineups/queue/lineup-phase.processor.ts` cron-firing-while-vote-in-flight race (Linear AC: "Vote submitted within 1s of deadline cron firing → vote counted or rejected? Document expected.") — the auto-advance integration spec covers the grace window and replacement-job race (ROK-1253), but the specific "vote POST hits API while phase-advance BullMQ job is mid-execution" timing window is not asserted anywhere. Behaviour is currently undocumented in the runbook because no test exercises it.
  Suggested: add an integration test that seeds a voting lineup with deadline ≈ now, fires `POST /lineups/:id/vote` concurrently with `lineup-phase.processor.processGraceAdvance()`, and asserts the contract — either vote rejected with a deterministic error code OR vote counted and embed reflects it. Document the expected behaviour in the runbook once the test passes.

- **[low]** `tools/test-bot/src/smoke/tests/lineup-abort.test.ts` — covers the admin-abort happy path + embed body, but does NOT explicitly assert the embed copy variance between `abort(reason)` vs `abort()` (no-reason). Linear AC ("Abort with reason vs without → embed copy differs correctly") was not walked end-to-end.
  Suggested: extend the existing companion bot smoke to call abort twice (once with reason, once without) and assert the embed body contains the reason line in one case and omits it in the other.
