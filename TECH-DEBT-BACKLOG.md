...
### 2026-05-19 — rok-1326-rl-infra-fleet-bugs (surfaced during `npx tsc --noEmit -p tools/mcp-rl-fleet/tsconfig.json`)

- **[med]** `tools/mcp-rl-fleet/tsconfig.json` — multiple `error TS2591: Cannot find name 'process' / 'Buffer' / 'NodeJS' / 'node:child_process'` errors across `src/exec.ts`, `src/index.ts`, and every tool file that imports from `node:*`. The package has no `@types/node` in its `package.json` and the workspace's hoisted `node_modules` doesn't expose `@types/node` to this package's tsc resolution either. Pre-existing on origin/main; reproduced by stashing all rok-1326 changes and re-running tsc. The build path (`tsc` from the workspace root, or whatever invokes the MCP server at runtime) presumably resolves types differently. Doesn't affect runtime — confirmed the MCP server boots and the fleet tools work end-to-end despite the typecheck noise.
  Suggested: add `"@types/node": "^20"` to `tools/mcp-rl-fleet/package.json` devDependencies (alongside the existing `typescript` dep) and re-run `npm install` at the repo root. Should be a one-line PR.
- **[med]** `tools/mcp-rl-fleet/src/index.ts:35`, `:101`, `:127`, `:177`, `:191` — `error TS2589: Type instantiation is excessively deep and possibly infinite`. Pre-existing on origin/main (independent of the missing-node-types issue above). Likely Zod schema chains with `.refine` + `.optional` produce types tsc can't terminate.
  Suggested: investigate which schemas are blowing the recursion depth and either flatten them or extract the inferred types to explicit interfaces. Lower urgency than the missing-types fix.
- **[low]** `tools/mcp-rl-fleet/src/tools/test-plan.ts:271` — `error TS2698: Spread types may only be created from object types`. Likely a `...(maybe)` spread where TS can't prove the operand is an object. Pre-existing; the test-plan code currently runs without issue at runtime. Touched in ROK-1326 Group 7 (X-Agent-Token header) but not at that line — Lead will look at it during reviewer phase.
...
### 2026-05-19 — rok-1326-rl-infra-fleet-bugs (surfaced during fleet validation `rl claim`)

- **[low]** `rl-infra/cli/rl:427` — `count=$(mutagen sync list 2>/dev/null | grep -c "^Name: $session$" || echo 0)` produces `count="0\n0"` when grep finds zero matches (grep outputs `"0"` AND exits 1, then `|| echo 0` appends another `"0"`). Subsequent `(( count == 1 ))` / `(( count > 0 ))` arithmetic expressions then print `((: 0\n0: syntax error in expression (error token is "0")` to stderr. Non-fatal — the script falls through to the create branch which works correctly — but it's noisy and the comment block above the line claims the construct works. Pre-existing on origin/main; reproduced by re-running `./rl-infra/cli/rl status --pretty` against a fleet with no sessions named that pattern. Surfaced during ROK-1326 fleet validation but the line itself isn't touched by this branch.
  Suggested: change `|| echo 0` to `|| true`. `grep -c` already emits a numeric stdout (including `"0"` on no match), so `|| true` keeps the pipeline exit clean without doubling the output.

### 2026-05-19 — rok-1326 (operator-flagged friction during fleet validation)

- **[high]** `tools/mcp-rl-fleet/src/tools/*` — every long-running MCP tool (`rl_validate_ci`, `rl_env_build_image_from_runner`, `rl_env_deploy`, `rl_env_clone_prod`) blocks the calling agent for the full duration (5-15 min each) because the server reports `execution.taskSupport: "forbidden"` (MCP SDK default; never opted in). The Bash tool auto-backgrounds equivalent calls; the MCP layer does not. Operator-flagged 2026-05-19 during ROK-1326 fleet validation: "the mcp tool call hold the leads attention for 5+ mins while they are running ci validations, this isnt ideal". **Filed as [ROK-1331](https://linear.app/roknua-projects/issue/ROK-1331).**

- **[high]** `rl-infra/cli/rl` + MCP wrapper — `RL_PROXMOX_HOST` defaults to `rl-infra.lan`. That hostname resolves only via the operator's LAN Pi-hole DNS (`192.168.0.9`). Agents whose DNS doesn't traverse the LAN (sandboxed contexts, MCP server inside a container, future cloud-based runs) can't reach the fleet at all without manually overriding `RL_PROXMOX_HOST=192.168.0.132`. Surfaced during ROK-1326: every `rl` invocation needed the override. **Open decision (not fixed):** (a) document the workaround in CLAUDE.md, (b) change default to the IP and accept the loss of LAN-rename flexibility, OR (c) add a fallback chain — try `.lan` name, on resolution failure retry against `$RL_INFRA_IP` from repo-root `.env`. Recommendation: (c) — least surprise.

- **[low]** `rl-infra/cli/rl::current_slot` / orchestrator `/srv/rl-infra/orchestrator/bin/claim` — when called for an agent that ALREADY holds a slot, the orchestrator returns the existing claim but reports `branch: "unknown"` (the value of `--branch` arg passed by the re-claim) instead of preserving the original branch from state. Cosmetic — claim data on disk is still correct — but confusing in `rl_status` output if `current_slot` is run after the initial claim. Surfaced during ROK-1326 fleet validation. Suggested: in `cmd_claim` / orchestrator, when an existing claim is detected, ignore the passed `--branch unknown` and return the persisted branch.

- **[low]** Background `Bash` invocations of `rl validate-ci --full` sometimes exit 0 with truncated output (~20 lines captured) instead of the full ~146+ line stream. Reproduced once during ROK-1326 (task `bygfh34md`); the immediately-following synchronous re-run produced full output cleanly. Could be Claude Code harness output-buffer flush timing OR a Mutagen-sync interaction. Non-blocking — re-running works — but worth understanding before relying on background `rl validate-ci` in unattended CI loops.

- **[HIGH — common agent-hit issue]** `scripts/sync-local-to-env.sh` doesn't restart the env's allinone container after writing new `app_settings` rows. The api's settings service caches decrypted settings on boot (30-min TTL). Sync inserts new rows but the running container keeps serving stale cache → Discord OAuth redirect shows `client_id=placeholder` + `localhost:3000` callback even though the env DB has the correct re-encrypted rows. **Operator note 2026-05-19: "common occurrence i see from agents using the fleet"** — agents repeatedly hit this, see broken OAuth, file false "fix-4 doesn't work" reports, then re-debug from scratch. The fact that fleet agents keep rediscovering this gap is the strongest signal it must be fixed in-script, not documented-around. Workaround: `ssh rl-infra "docker restart rl-env-<slug>-allinone"` then wait ~12s for health. Fix: add an explicit container restart step at the END of sync-local-to-env.sh (after the re-encrypt path + admin bootstrap). Same surface as the operator's local CLAUDE.md clone-prod-to-local runbook Step 4 (which already says "Bounce the API to invalidate the settings cache"). Use `ssh_run run-on-runner` pattern or a new `/srv/rl-infra/orchestrator/bin/env-restart <slug>` binary so it works for rl-agent (matches the broader rl-agent-vs-rl gap above).

- **[HIGH]** Settings-mode sync leaves admin identity + Discord identity unlinked, separate from operator's local/prod experience. On prod/local the operator's user row has `discord_id` populated, so Discord OAuth finds the existing user and the same account is "admin" + "Discord-authed". On fleet `settings` mode: only `app_settings` (+ a couple others) are synced — the `users`, `local_credentials`, and Discord-identity link tables are excluded (FK chains break in a fresh env, same reason clone-prod-to-local sanitization excludes them). Result: `admin@local` is a freshly-bootstrapped user with no Discord link; the operator's first Discord login on the env creates a SEPARATE user keyed on Discord ID. Operator-flagged 2026-05-19 during ROK-1326 fleet retest. Fix sketch: in `settings` mode, ALSO sync (a) the operator's own user row from `users` (identifiable by their Discord ID or admin role) AND (b) the corresponding row in whichever table links Discord IDs (likely `users.discord_id` column or a separate `discord_identities`). Bootstrap-admin would then UPDATE the linked user's password (not INSERT a new admin@local) so the env's admin login + Discord login resolve to the same row. Alternative quick fix: document that fleet envs require `full` mode (clone_prod=true) for unified identity — but that's a heavyweight workaround.

- **[HIGH — STRICT, must fix before ROK-1331]** `scripts/sync-local-to-env.sh` requires `RL_PROXMOX_USER=rl` (operator mode) but a fresh agent following `./rl-infra/cli/rl` flows runs under `RL_PROXMOX_USER=rl-agent`. The script's `docker inspect` calls (lines 64, 248) AND `docker exec`-into-pg pipe (lines 105, 141, 197, 352) need docker socket access on the VM — `rl-agent` is intentionally not in the docker group (the Docker API is filtered via socket-proxy for the orchestrator binaries only). Symptom for a fresh agent: `ERROR: env DB container 'rl-env-<slug>-pg' is not running` even though the container IS up — the docker inspect via ssh returned empty under rl-agent.
  
  This is the SAME root cause as Bug R(2)'s scaffold issue (fixed in `b6c1303b` by routing through `run-on-runner`). Same fix shape: add orchestrator binaries `/srv/rl-infra/orchestrator/bin/env-inspect <slug>` (returns `{running, slot, allinone_status, pg_status}` JSON) and `/srv/rl-infra/orchestrator/bin/env-psql <slug> [args]` (wraps `docker exec -i rl-env-<slug>-pg psql ...` with stdin passthrough). Then sync-local-to-env.sh uses `ssh_run env-inspect <slug>` and `ssh_run env-psql <slug> ...` instead of raw `ssh "docker ..."`.
  
  Operator-mode override (`RL_PROXMOX_USER=rl`) works as a workaround for hand-invocation today, but breaks the "fresh agent runs the documented flow" path. The MCP `rl_env_sync_from_local` tool happens to work because `env-sync.ts` inherits `process.env` without setting `RL_PROXMOX_USER` — operators who don't export the var in their shell get the script's `"${RL_PROXMOX_USER:-rl}"` default. Fragile: the moment an agent context sets `RL_PROXMOX_USER=rl-agent` in env (which CLAUDE.md actually documents as the expected agent default), the MCP path breaks too.
  
  Operator priority 2026-05-19: "this entire process needs to work for fresh agents". Recommend this gets a Linear story before ROK-1331's task-style execution refactor, because the new task-tracking binaries will be in the same orchestrator/bin/ surface and should land together.

- **[med]** Web vitest coverage race on fleet runner: `validate-ci.sh --full` Unit tests step FAILs with `"Something removed the coverage directory '/workspace/web/coverage/.tmp' Vitest created earlier. Make sure you are not running multiple Vitests with the same coverage.reportsDirectory"`. Reproduced on ROK-1326's run against slot-1. The api workspace's 6092 tests across 448 suites pass cleanly first, then the web vitest step trips on the coverage tmp dir disappearing mid-run. Plausibly Mutagen-vs-vitest race: vitest worker creates `.tmp/`, Mutagen one-way-replica sync ticks notice it doesn't exist on the laptop (alpha), and DELETES it on the runner (beta) — even though the parent `coverage` is in the ignore list, the deletion semantics of `--mode=one-way-replica` may still affect newly-created subdirs that the ignore matches at root level. ROK-1326 didn't touch web/vitest config; ROK-1326 also didn't introduce coverage in the ignore list (it was already there). Could ALSO be vitest's own multi-worker locking failing on a network filesystem. Suggested investigation: (a) reproduce on slot-2 immediately after a clean claim, (b) try `--ignore='**/coverage/**'` (recursive) in the mutagen sync config to be sure subdir creates aren't replicated up, (c) try `vitest run --coverage --pool=forks --poolOptions.forks.singleFork=true` to eliminate multi-worker locking. Workaround: skip coverage on fleet (`vitest run` without `--coverage`) — local + GitHub CI still catch coverage threshold.
