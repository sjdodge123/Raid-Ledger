...
- **[high]** `tools/mcp-rl-fleet/src/tools/*` — every long-running MCP tool (`rl_validate_ci`, `rl_env_build_image_from_runner`, `rl_env_deploy`, `rl_env_clone_prod`) blocks the calling agent for the full duration (5-15 min each) because the server reports `execution.taskSupport: "forbidden"` (MCP SDK default; never opted in). The Bash tool auto-backgrounds equivalent calls; the MCP layer does not. Operator-flagged 2026-05-19 during ROK-1326 fleet validation: "the mcp tool call hold the leads attention for 5+ mins while they are running ci validations, this isnt ideal".
  Suggested: a dedicated story to add task-style execution. Sketch: long tools return `{task_id, log_url}` immediately after spawning the work as a detached process on the VM. Sister tools `rl_task_status(task_id)` (cheap poll) + `rl_task_wait(task_id)` (long-poll via inotifywait, same pattern as `rl_test_plan_wait`). Persist task state under `/srv/rl-infra/state/tasks/<task_id>.json` so MCP server restarts don't lose tracking. Add cancellation via `rl_task_cancel(task_id)`. Affected tools list above + their orchestrator binaries on the VM. Workaround until this lands: agents can call `./rl-infra/cli/rl validate-ci --full` via Bash (the harness auto-backgrounds Bash, just not MCP).
