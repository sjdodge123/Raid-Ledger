...
- **[med]** rl-infra fleet — Mutagen worktree sync may not fire (or may complete much later than expected) on a fresh claim. PR-1 round-3 probe Phase A reproduced: `rl_claim` returned slot 2 at 20:38:35 against branch `rok-1338-mcp-readonly-surface` (HEAD `4d8666ec`), but `rl_status` 6 minutes later showed `runner-2.last_sync_at = 2026-05-21T20:01:51Z` (37 min stale) and `runner-2.worktree_head = 61d348b` (a sibling branch's HEAD, NOT this claim's HEAD). `build_image` hung the full 6 min without progress before operator-cancel. Without `rl_status.last_sync_at` + `worktree_head` (added by this same PR-1), this failure mode was invisible from MCP — would manifest only as "build hangs forever."
  Suggested: investigate whether Mutagen daemon is even running on claim, OR whether the sync session creation is async and not awaited by `rl_claim`. Mid-term: `rl_claim` should block until first sync round completes (or expose `claim_wait_for_sync` flag). Surfaced via PR-1's new rl_status fields — this finding is also a validation that those fields work.
