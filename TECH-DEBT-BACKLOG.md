...
- **[med]** `web/src/pages/admin-settings-page.tsx` — dead code. `AdminSettingsPage` is exported (line 37) but never imported. The file mounts `<UpdateBanner />` (line 102) but is unreachable; the real `/admin/settings` route binds `web/src/components/admin/admin-settings-layout.tsx` (`app-routes.tsx:60`). ROK-1242's spec described this file as the live mount, which is why the banner appeared "broken" pre-fix even though the contract + service + component were all correct. Fixed inline by mounting UpdateBanner in the live layout; the dead file remains.
  Suggested: delete `web/src/pages/admin-settings-page.tsx` entirely. Verify nothing imports it (`grep -rn "admin-settings-page\\|AdminSettingsPage" web/src` returns only the file's own export). Single-file removal, no test impact since nothing references it.
...
### 2026-05-14 — fix/batch-2026-05-14 (surfaced during operator-triggered /security-review on whole repo + local env)

Codex CLI returned clean for the branch diff itself. These findings are from a parallel whole-repo Claude sweep + local-env / docker-compose audit. None are exploitable in the current threat model; all are defense-in-depth or local-dev-exposure issues. Full report: `planning-artifacts/security-review-7bec0fb2-fullrepo.md` and `…-summary.md`.

**Local env / docker-compose (highest operational urgency — local DB now contains a sanitized prod data clone):**

- **[high]** `docker-compose.yml:18-19` — Postgres `db` service publishes `"5432:5432"`, which Docker binds to `0.0.0.0:5432`. Credentials are hardcoded `user`/`password` (line 9-10). Anyone on the same LAN/wifi can connect to the local DB, which currently holds a real prod data clone (114 users with Discord IDs, 82 events, 1918 games, 52 `app_settings` rows including encrypted API keys). Verified with `docker port raid-ledger-db` → `0.0.0.0:5432, [::]:5432`.
  Suggested: change to `"127.0.0.1:5432:5432"`. Same one-line fix applies to the `redis` ports entry.
- **[high]** `docker-compose.yml:33` — Redis `redis` service publishes `"6379:6379"` → `0.0.0.0:6379`, no auth password set. Same LAN-exposure as the Postgres finding; less PII at risk but BullMQ payloads and queue control are reachable.
  Suggested: change to `"127.0.0.1:6379:6379"`.
- **[med]** `docker-compose.yml:66` — `api` test-profile service mounts `/var/run/docker.sock:/var/run/docker.sock` into the container. When run via `docker compose --profile test up`, the container has root-equivalent host access (can manage every container, image, volume). Profile-gated so daily dev (host npm processes) is unaffected, but the security assumption is invisible in the file.
  Suggested: drop the mount unless the test profile genuinely needs it. If kept, add an inline comment explaining why and what calls it.
- **[med]** `docker-compose.yml:57` — `JWT_SECRET: "${JWT_SECRET:-dev-secret-change-in-prod}"`. The allinone Dockerfile (`Dockerfile.allinone:317-336`) refuses to start with the OLD literal `raid-ledger-default-secret-change-in-production`, but does NOT guard this newer compose-file default. Anyone running this `docker-compose.yml` outside true dev would start with a known JWT signing key.
  Suggested: extend the `start-api.sh` `HARDCODED_DEFAULT` guard to a list including `dev-secret-change-in-prod`, OR have the compose file fail if `JWT_SECRET` is unset and `NODE_ENV !== "development"`.

**Whole-repo audit (Claude sub-agent, 79 controllers reviewed; full detail in `…-fullrepo.md`):**

- **[high]** `api/src/admin/branding.controller.ts:25, 152-164` + `api/src/main.ts:59-65` — SVG entry in `ALLOWED_TYPES` declares `magic: [0x3c]`, a single `<` byte. Any text file beginning with `<` passes the check; uploaded SVG is written as `logo.svg` and served from same origin under `/uploads/branding/logo.svg`. Helmet's app-wide CSP doesn't extend per-asset. Admin-only upload limits the threat to admin-trojan / social-engineering navigation, but a logo SVG containing `<script>` executes JS in the API origin when navigated directly. (Avatar upload path is hardened via `sharp` re-encode — branding intentionally preserves SVG.)
  Suggested: drop SVG from `ALLOWED_TYPES`. If SVG is required, sanitize server-side (`svg-sanitizer` / `DOMPurify`), serve with `Content-Disposition: attachment` AND a per-static-asset CSP `default-src 'none'; style-src 'unsafe-inline'`, and consider a sandboxed subdomain for branding assets.
- **[med]** `api/src/ai/ai-providers.controller.ts:60-73, 234-250` — `@Body() body: AiProviderConfigDto` is a TS type alias only; no `ValidationPipe` and no Zod parse. `body.apiKey` / `body.url` / `body.model` are persisted via `settings.set()` as-is. Admin-gated so external impact is low, but data-integrity / settings-cache corruption / log tampering are possible from within an admin session.
  Suggested: add `AiProviderConfigSchema` to `packages/contract` and `.parse(body)` at the controller, OR convert the DTO to a class-validator class with a paired `ValidationPipe`.
- **[med]** `api/src/main.ts:23-46` (`installAutoClientUrlDetection`) — When `CORS_ORIGIN === 'auto'` and `CLIENT_URL` is unset, the first request through the middleware writes `process.env.CLIENT_URL` from the unverified `Host` + `X-Forwarded-Proto` headers. There is no allow-list. In a direct-access deployment (no nginx normalization), one crafted request can poison `CLIENT_URL` for the lifetime of the process — and `CLIENT_URL` flows into OG-embed redirects, OAuth callbacks, and Discord magic-link bodies sent to all subsequent users. Synology + nginx topology mitigates in practice, but the code does not enforce that contract.
  Suggested: in `auto` mode, require an `ALLOWED_HOSTS` env var (comma-separated allow-list) and reject non-matching `Host` headers BEFORE mutating env. Better: derive `CLIENT_URL` per-request via a request-scoped helper instead of mutating `process.env`.
- **[med]** `api/src/auth/magic-link.service.ts:36-49` + `web/src/App.tsx:42-56` — `MagicLinkService.generateLink` produces a 15-minute JWT with `sub`, `username`, `role`, `magicLink: true` and ships it as `?token=` in a URL sent to Discord. The frontend strips the query AFTER the page loads (already too late — Discord link-tracker, browser history, HTTPS proxies, and Referer headers may have logged it). The `magicLink: true` claim is NOT enforced anywhere downstream — the JWT is interchangeable with a normal login token. No single-use enforcement: a captured magic link grants 15 minutes of full account impersonation.
  Suggested: store `jti` in Redis at issue (`setex(jti, 900, '1')`), `getdel` on consumption, reject if absent. Add a magic-link-only guard that requires `magicLink === true` and refuses write endpoints. Best long-term: send the link to a server-side `/auth/redeem-magic-link` POST instead of GET-with-query.
- **[med]** `api/src/settings/encryption.util.ts:32-37` (`deriveKey`) — Salt is built as `Buffer.from(secret.slice(0, 32).padEnd(32, '0'))`. Salt derived deterministically from the secret defeats the salt's purpose. Acceptable for the per-process JWT_SECRET use-case in isolation; dangerous if reused for password hashing. Also: short JWT_SECRET values get padded with literal `'0'` characters, weakening entropy contribution.
  Suggested: persist a process-local random 32-byte salt in `app_settings` (`encryption_salt`) and pass it explicitly. At minimum add a code comment marking this pattern as JWT-secret-only and forbidding reuse for password storage.
- **[low]** `api/src/steam/steam-auth.controller.ts:233-255` + `api/src/steam/steam-http.util.ts:49-81` — Steam-OpenID nonce replay protection currently lives at Steam (`mode=check_authentication` is supposed to return `is_valid:false` for previously-checked nonces). We don't cache `openid.response_nonce` ourselves; depends on Steam's nonce cache duration. Replay risk is low (only re-links the same Steam ID to the same user), but the layered-defense story is missing.
  Suggested: after successful link, `redis.setex(\`steam-openid:\${nonce}\`, 86400, '1')` and reject if already present.
- **[low]** `api/src/auth/jwt.strategy.ts:28` — `process.env.JWT_SECRET!` non-null assertion at construction. If the env var is missing in production, passport-jwt receives `undefined` as `secretOrKey` and may either crash at boot or accept tokens signed with an empty key. `encryption.util` validates banned values, but `JwtStrategy` does not enforce minimum entropy / non-empty.
  Suggested: throw at module init if `JWT_SECRET` is missing or shorter than 32 chars (fail-closed).
- **[low]** `api/src/auth/auth.controller.ts:48-67` (`exchangeCode`) — `redis.get` then `redis.del` is not atomic. Under heavy concurrency two simultaneous requests with the same code could both retrieve the token, both `del`, both succeed. Single-use semantics are claimed in a code comment but not enforced.
  Suggested: replace with `redis.getdel(key)` (Redis 6.2+) or wrap in `MULTI/EXEC`.
- **[low]** `api/src/csp-report/csp-report.controller.ts:14-32` — Logs full CSP report payload to structured logger (`this.logger.log({ event: 'csp_violation', report })`). 64kb body limit caps abuse but an attacker who can produce CSP violations can spam logs with arbitrary content reaching the configured log sink.
  Suggested: cap and redact (`JSON.stringify(report).slice(0, 4096)`), or rely on Sentry tags + sampling and skip the second log line.
- **[low]** `api/src/plugins/wow-common/blizzard-equipment.helpers.ts:25-52` (`fetchSingleIconUrl`) — Sends `Bearer ${token}` Authorization header to a URL pulled from a parent Blizzard JSON response (`equipped_items[].media.key.href`) without host validation. If a future Blizzard endpoint returns a non-Blizzard URL (or MITM injects one), the bearer leaks to a third-party host. Likelihood low (HTTPS to Blizzard, Blizzard owns the response) but the defensive check is one line.
  Suggested: assert `new URL(mediaHref).hostname.endsWith('blizzard.com') || .endsWith('battle.net')` before fetch.

**Branch diff (`fix/batch-2026-05-14`):**

- **[low]** `api/src/version/version-check.service.ts:144` → `web/src/components/admin/UpdateBanner.tsx:48` — `latestReleaseUrl` flows from GitHub's `body.html_url` through `app_settings` to a JSX `<a href={href} target="_blank" rel="noopener noreferrer">`. Zod's `z.string().url()` allows `javascript:` URLs (it uses the `URL` constructor which accepts them), so the contract layer doesn't block scheme-based XSS. Trusted source today; defense-in-depth one-liner.
  Suggested: validate `href.startsWith('https://github.com/')` either at the storage step in `storeVersionCheckResults` (reject otherwise, store empty) OR at render time in `BannerContent`.
