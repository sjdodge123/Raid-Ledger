...
- **[low]** `rl-infra/orchestrator/bin/env-spin` idempotent path (lines 107-272) — only ONE `LAST_PHASE="idempotent_path_entered"` marker. New-env path has 12 markers; if a failure happens between the guarded `state::mutate` and `audit::log idempotent`, the EXIT trap can only point to the entry. devedup-rl reviewer: recommends 2-3 more (`idempotent_state_mutated`, `idempotent_traefik_written`, `idempotent_bootstrap_done`) to match the new-env granularity.
