...
3. **Fetch QA test cases from Linear (your primary test plan):**
   - `ToolSearch("+linear list_comments")`
   - `mcp__linear__list_comments(issueId: "<ISSUE_ID>")`
   - Find the comment titled "## Manual Testing Checklist" — this was generated by the QA agent
   - Use this checklist as your primary test plan. Each checklist item maps to a specific test you should run.
   - If no QA checklist is found, fall back to the acceptance criteria listed above.
...
Work through the QA test cases from Linear as your primary test plan. For each test case:
