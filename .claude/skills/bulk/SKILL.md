...
- Tech debt, chores, performance improvements
...
Step 2: Implement → Batch branch, worktrees, plan complex stories, parallel devs, merge into batch branch
...
3. **Unit tests** — all workspaces pass
4. **Integration tests** — `npm run test:integration -w api`
5. **CI** — build + type check + lint (validated as part of gate 3-4 process)
...
2. Each dev gets a worktree on `batch/rok-<num>` branched from the batch branch
3. As devs complete, lead merges each `batch/rok-<num>` into `batch/YYYY-MM-DD`
...
1. **Read `planning-artifacts/batch-state.yaml`** — contains full pipeline state
...
| **standard** | Single-module change, straightforward | Yes |
...
- Touches `Dockerfile*`, `docker-entrypoint.sh`, or `nginx/` → **NOT eligible for bulk** — infrastructure changes require their own PR with local container validation (see CLAUDE.md)
...
| Dev | `templates/dev-bulk.md` | Step 2d (one per story) | opus | Per-story |
| Reviewer | `devedup-rl:reviewer` subagent | Step 3a (once, on batch branch diff) | sonnet | Once per batch |

4 agent types total: Lead + Planner (conditional) + Dev + Reviewer.
