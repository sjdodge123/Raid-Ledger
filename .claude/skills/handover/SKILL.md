...
- **Lead context discipline** (CLAUDE.md section of that name): hand over at ~300–350k context at a natural seam, never past ~450k (measure it from the transcript's `usage`, don't guess); the Lead does not poll — gates, deploys, CI and PR babysitting go to one `sonnet` ops lane that replies once with PASS/FAIL; batch tool calls, no "still running" turns; keep log tails and bulk reads out of the Lead's context. The Lead's cost is turns × context size (2026-09-19: 499M tokens re-read by one 12-hour Lead vs 80M by every sub-agent combined).
