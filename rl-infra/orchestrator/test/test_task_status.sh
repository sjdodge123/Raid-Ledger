...
test_task_status_mid_run() {
    ...
    local out exit_code
    out=$(bash "$BIN_DIR/task-status" "$task_id" 2>&1)
    exit_code=$?
...
test_task_status_succeeded_terminal() {
    ...
    while [[ "$status" != "succeeded" && "$status" != "failed" ]] && (( attempts < 50 )); do
        status=$(bash "$BIN_DIR/task-status" "$task_id" 2>/dev/null | jq -r '.status' 2>/dev/null || echo "")
        attempts=$((attempts + 1))
        sleep 0.1
    done
    ...
    exit_code=$(bash "$BIN_DIR/task-status" "$task_id" 2>/dev/null | jq -r '.script_exit_code' 2>/dev/null || echo "")
    finished_at=$(bash "$BIN_DIR/task-status" "$task_id" 2>/dev/null | jq -r '.finished_at' 2>/dev/null || echo "")
...
test_task_status_failed() {
    ...
    bash "$BIN_DIR/task-start" "$task_id" --tool manual --slot 1 -- /bin/sh -c "exit 7" >/dev/null 2>&1 || true
...
test_task_status_log_tail() {
    ...
    bash "$BIN_DIR/task-start" "$task_id" --tool manual --slot 1 -- /bin/sh -c "echo hello-from-task; exit 0" >/dev/null 2>&1 || true
    ...
    local log_tail
    log_tail=$(bash "$BIN_DIR/task-status" "$task_id" 2>/dev/null | jq -r '.log_tail' 2>/dev/null || echo "")
...
test_task_status_rejects_invalid_id() {
    ...
    local exit_code=0
    bash "$BIN_DIR/task-status" "BAD@ID" >/dev/null 2>&1 || exit_code=$?
...
test_task_status_rejects_missing_file() {
    ...
    local exit_code=0
    bash "$BIN_DIR/task-status" "ghostmiss" >/dev/null 2>&1 || exit_code=$?
...
test_task_status_steps_field_present() {
    ...
    bash "$BIN_DIR/task-start" "$task_id" --tool manual --slot 1 -- /bin/sleep 0.5 >/dev/null 2>&1 || true

    local steps_type
    steps_type=$(bash "$BIN_DIR/task-status" "$task_id" 2>/dev/null | jq -r '.steps | type' 2>/dev/null || echo "missing")
